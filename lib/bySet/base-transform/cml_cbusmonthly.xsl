...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Columbus Monthly</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="cml_cbusmonthly"/>         <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="cml_cbusmonthly"/>         <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dcterms:spatial"          mode="cml_cbusmonthly"/>         <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="cml_cbusmonthly"/>         <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="cml_cbusmonthly"/>         <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="odn"/>                     <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cml_cbusmonthly"/>         <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_cbusmonthly"/>         <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->

      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_cbusmonthly"/>
      <xsl:apply-templates select="dcterms:issued"           mode="cml_cbusmonthly"/>
      <xsl:apply-templates select="dc:coverage"              mode="cml_cbusmonthly"/>
      <xsl:apply-templates select="dcterms:medium"           mode="cml_cbusmonthly"/>
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_cbusmonthly"/>
      <xsl:apply-templates select="dcterms:mediator"         mode="cml_cbusmonthly"/>
      <xsl:apply-templates select="dcterms:references"       mode="cml_cbusmonthly"/>


    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:rights"  mode="cml_cbusmonthly"/>
  <xsl:template match="dc:publisher" mode="cml_cbusmonthly"/>
  <xsl:template match="dcterms:issued" mode="cml_cbusmonthly"/>
  <xsl:template match="dcterms:temporal" mode="cml_cbusmonthly"/>
  <xsl:template match="dcterms:medium" mode="cml_cbusmonthly"/>
  <xsl:template match="dcterms:isPartOf" mode="cml_cbusmonthly"/>
  <xsl:template match="dcterms:provenance" mode="cml_cbusmonthly"/>
  <xsl:template match="dcterms:mediator" mode="cml_cbusmonthly"/>
  <xsl:template match="dcterms:references" mode="cml_cbusmonthly"/>
  <xsl:template match="dc:contributor" mode="cml_cbusmonthly"/>

  <xsl:template match="dc:identifier" mode="cml_cbusmonthly">
    <xsl:choose>
  ...
  <xsl:template match="dc:coverage" mode="cml_cbusmonthly">
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_cbusmonthly">
    <xsl:for-each select="tokenize(., ';')">
