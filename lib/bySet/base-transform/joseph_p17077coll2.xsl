...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">St. Joseph’s Orphanage and Trade School, Columbus, Ohio (1877-1888)</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="joseph_p17077coll2"/>      <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="joseph_p17077coll2"/>      <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="joseph_p17077coll2"/>      <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="joseph_p17077coll2"/>      <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="joseph_p17077coll2"/>      <!-- create dcterms:publisher                                   -->
      ...
      <xsl:apply-templates select="dc:source"                mode="joseph_p17077coll2"/>      <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="joseph_p17077coll2"/>

  <xsl:template match="dc:publisher" mode="joseph_p17077coll2">
  ...
  <xsl:template match="dc:date" mode="joseph_p17077coll2">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dcterms:spatial" mode="joseph_p17077coll2">
  ...
  <xsl:template match="dc:creator" mode="joseph_p17077coll2">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="joseph_p17077coll2">
    <xsl:choose>
