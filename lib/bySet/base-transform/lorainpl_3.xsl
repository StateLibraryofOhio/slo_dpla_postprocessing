...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Lorain Public Library System</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Avon Yearbooks</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="lorainpl_3"/>              <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="lorainpl_3"/>              <!-- create dc:format                                           -->
  ...
  <xsl:template match="dc:format" mode="lorainpl_3">
    <xsl:if test="not(preceding-sibling::dc:format/text() = current()/text())">
  ...
  <xsl:template match="dc:identifier" mode="lorainpl_3">
    <xsl:choose>
      <xsl:when test="(starts-with(., 'http://') or starts-with(., 'https://')) and (contains(., 'loraincountymemory.org/items'))">
