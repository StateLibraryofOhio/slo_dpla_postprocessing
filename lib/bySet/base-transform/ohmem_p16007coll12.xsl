...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">State Library of Ohio Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="ohmem_p16007coll12"/>      <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dcterms:alternative"      mode="ohmem_p16007coll12"/>      <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="ohmem_p16007coll12"/>      <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="ohmem_p16007coll12"/>      <!-- create dcterms:extent                                      -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="odn"/>                     <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="ohmem_p16007coll12"/>      <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dcterms:issued"           mode="ohmem_p16007coll12"/>      <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:abstract"         mode="ohmem_p16007coll12"/>      <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:abstract" mode="ohmem_p16007coll12"/>

  <xsl:template match="dcterms:issued" mode="ohmem_p16007coll12"/>

  <xsl:template match="dc:relation" mode="ohmem_p16007coll12"/>

  <xsl:template match="dcterms:alternative" mode="ohmem_p16007coll12">
  ...
  <xsl:template match="dcterms:extent" mode="ohmem_p16007coll12">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="ohmem_p16007coll12">
    <xsl:choose>
  ...
  <xsl:template match="dc:contributor" mode="ohmem_p16007coll12">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
