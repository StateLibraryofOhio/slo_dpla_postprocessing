...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="odn_templates.xsl"/>
    ...
    <xsl:template match="//oai_qdc:qualifieddc">
      ...
      <xsl:apply-templates select="dc:language"          mode="midpointe_p16488coll11"/>    <!-- create dcterms:language                 -->
      ...
      <xsl:apply-templates select="dcterms:created"      mode="midpointe_p16488coll11"/>    <!-- create dc:date                          -->
      <xsl:apply-templates select="dc:format"            mode="midpointe_p16488coll11"/>    <!-- create dc:format                        -->
      <xsl:copy-of         select="dcterms:spatial"      copy-namespaces="no"/>             <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:coverage"          mode="midpointe_p16488coll11"/>    <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:subject"           mode="midpointe_p16488coll11"/>    <!-- create dcterms:subject                  -->
      ...
      <xsl:apply-templates select="dc:source"            mode="midpointe_p16488coll11"/>    <!-- create dc:relation                                                         -->
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll11">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:subject" mode="midpointe_p16488coll11">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:source" mode="midpointe_p16488coll11">
    <xsl:element name="dc:relation" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:coverage" mode="midpointe_p16488coll11">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:created" mode="midpointe_p16488coll11">
