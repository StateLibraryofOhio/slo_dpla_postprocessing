...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Gay Ohio History Initiative Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="ohmem_p267401coll30"/>     <!-- create edm:rights    and dc:rights                         -->
      <xsl:apply-templates select="dcterms:license"          mode="ohmem_p267401coll30"/>     <!-- create edm:rights                                          -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="ohmem_p267401coll30"/>     <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="ohmem_p267401coll30"/>     <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dc:source"                mode="ohmem_p267401coll30"/>     <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dcterms:license" mode="ohmem_p267401coll30">
    <xsl:element name="edm:rights" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:temporal" mode="ohmem_p267401coll30">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
