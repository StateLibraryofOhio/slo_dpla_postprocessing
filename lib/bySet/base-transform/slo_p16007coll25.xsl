...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Treasurer of the State of Ohio Annual Reports</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="ohmem_p16007coll25"/>      <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="ohmem_p16007coll25"/>      <!-- create dcterms:extent                                      -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="ohmem_p16007coll25"/>      <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="ohmem_p16007coll25"/>      <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="ohmem_p16007coll25"/>      <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:requires"         mode="ohmem_p16007coll25"/>      <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:mediator"         mode="ohmem_p16007coll25"/>      <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dcterms:extent" mode="ohmem_p16007coll25">
  ...
  <xsl:template match="dc:publisher" mode="ohmem_p16007coll25">
    <xsl:element name="dcterms:publisher" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="replace(normalize-space(.), '\]', '')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="ohmem_p16007coll25">
    <xsl:choose>
  ...
  <xsl:template match="dc:relation" mode="ohmem_p16007coll25"/>

  <xsl:template match="dcterms:created" mode="ohmem_p16007coll25"/>

  <xsl:template match="dcterms:requires" mode="ohmem_p16007coll25"/>

  <xsl:template match="dcterms:mediator" mode="ohmem_p16007coll25"/>
