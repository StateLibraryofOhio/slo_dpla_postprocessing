...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Daisy Wheaton Stereographs</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="delawaremem_daisy"/>       <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="delawaremem_daisy"/>       <!-- create edm:rights    and dc:rights                         -->
  ...
  <xsl:template match="dc:identifier" mode="delawaremem_daisy">
    <xsl:choose>
  ...
  <xsl:template match="dc:language" mode="delawaremem_daisy">
    <xsl:if test="(normalize-space(.) = 'English') or (normalize-space(.) = 'Engliah')">
      <xsl:element namespace="http://purl.org/dc/terms/" name="dcterms:language">English</xsl:element>
    </xsl:if>
  </xsl:template>

  <xsl:template match="dc:rights" mode="delawaremem_daisy">
  ...
  <xsl:template match="dc:subject" mode="delawaremem_daisy">
    <xsl:for-each select="tokenize(., '&#xA;')">
