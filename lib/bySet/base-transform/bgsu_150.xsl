...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Ohio historical pamphlets, publications, and ephemera</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_150"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_150"/>
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_150"/>
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="bgsu_150"/>
      <xsl:apply-templates select="dcterms:references"       mode="bgsu_150"/>

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:source" mode="bgsu_150"/>

  <xsl:template match="dcterms:created" mode="bgsu_150"/>

  <xsl:template match="dcterms:isReferencedBy" mode="bgsu_150"/>

  <xsl:template match="dcterms:references" mode="bgsu_150"/>

  <xsl:template match="dc:identifier" mode="bgsu_150">
    <xsl:choose>
