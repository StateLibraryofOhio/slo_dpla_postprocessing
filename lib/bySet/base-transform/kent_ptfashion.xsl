...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Pauline Trigère Fashion Design Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_ptfashion"/>           <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_ptfashion"/>           <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_ptfashion"/>           <!--  throw into dc:relation                                    -->
  ...
  <xsl:template match="dc:description" mode="kent_ptfashion">
    <xsl:choose>
      <xsl:when test="(normalize-space(.) != '') and not (starts-with(., 'http'))">
        <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="normalize-space(substring-before((concat(replace(replace(replace(replace(replace(., '&lt;[/]*span&gt;', ''), '&lt;a href=&quot;', ' '), '&amp;nbsp;', ' '), '&lt;[/]*p&gt;', ' '), '&quot;&gt;', '|'), '|')), '|'))"/>
        </xsl:element>
      </xsl:when>
