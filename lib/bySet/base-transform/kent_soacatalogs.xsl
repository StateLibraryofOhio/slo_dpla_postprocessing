...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">School of Art Galleries Exhibition Catalogs</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_soacatalogs"/>        <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="kent_soacatalogs"/>        <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_soacatalogs"/>        <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="odn"/>                     <!-- create dcterms:extent                                      -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="kent_soacatalogs"/>        <!-- create dcterms:publisher                                   -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_soacatalogs"/>        <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_soacatalogs"/>

  <xsl:template match="dc:publisher" mode="kent_soacatalogs"/>

  <xsl:template match="dc:format" mode="kent_soacatalogs"/>

  <xsl:template match="dc:description" mode="kent_soacatalogs">
    <xsl:choose>
      ...
      <xsl:otherwise>
         <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
           <xsl:value-of select="normalize-space(replace(replace(replace(., '&amp;nbsp;', ' '), '&lt;br /&gt;', ''), '&lt;[/]*p&gt;', ''))"/>
         </xsl:element>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="kent_soacatalogs">
