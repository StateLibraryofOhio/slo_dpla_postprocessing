...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Cincinnati-Area Church and Cemetery Records</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:language"              mode="plochc_p16998coll55"/>     <!-- create dcterms:language                                    -->
      <xsl:apply-templates select="dc:creator"               mode="odn"/>                     <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="plochc_p16998coll55"/>     <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="plochc_p16998coll55"/>     <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dcterms:alternative"      mode="plochc_p16998coll55"/>     <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="plochc_p16998coll55"/>     <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="plochc_p16998coll55"/>     <!-- create dcterms:extent                                      -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:copy-of         select="dcterms:isReferencedBy"   copy-namespaces="no"/>           <!-- create IIIF metadata                                       -->
      <xsl:apply-templates select="dc:publisher"             mode="plochc_p16998coll55"/>     <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="plochc_p16998coll55"/>     <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="odn"/>                     <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:apply-templates select="dcterms:rightsHolder"     mode="plochc_p16998coll55"/>     <!-- create dcterms:rightsHolder                                -->
      ...
      <xsl:apply-templates select="dcterms:tableOfContents"  mode="plochc_p16998coll55"/>
      <xsl:apply-templates select="dcterms:created" mode="plochc_p16998coll55"/>
      <xsl:apply-templates select="dcterms:medium" mode="plochc_p16998coll55"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:tableOfContents"  mode="plochc_p16998coll55"/>
  <xsl:template match="dcterms:license"  mode="plochc_p16998coll55"/>
  <xsl:template match="dcterms:created"  mode="plochc_p16998coll55"/>
  <xsl:template match="dc:publisher"  mode="plochc_p16998coll55"/>
  <xsl:template match="dcterms:extent"  mode="plochc_p16998coll55"/>
  <xsl:template match="dc:format"  mode="plochc_p16998coll55"/>
  <xsl:template match="dc:relation"  mode="plochc_p16998coll55"/>
  <xsl:template match="dcterms:rightsHolder"  mode="plochc_p16998coll55"/>

  <xsl:template match="dc:contributor" mode="plochc_p16998coll55">
  ...
  <xsl:template match="dcterms:medium" mode="plochc_p16998coll55">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="(normalize-space(.) != '')">
  ...
  <xsl:template match="dc:language" mode="plochc_p16998coll55">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:date" mode="plochc_p16998coll55">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
