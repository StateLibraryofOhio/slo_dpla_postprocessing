...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">An Evening with Judge Florence Allen</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_allen"/>              <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_allen"/>              <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_allen"/>              <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_allen"/>

  <xsl:template match="dc:description" mode="kent_allen">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test="not(normalize-space(.)='')">
          <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
            <xsl:value-of select="normalize-space(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(., '&lt;a href=&quot;http://digitalcommons.kent.edu/allen&quot; &gt;', ''), '&lt;a href=&quot;http://www.starbeacon.com/community/a-legacy-of-education/article_ff4079cd-7478-5858-8fd8-092e31f87b84.html&quot; &gt;', ''), '&lt;a href=&quot;http://www.hscl.cr.nps.gov/insidenps/report.asp\?STATE=CO&amp;PARK=ROMO&amp;SORT=5&amp;RECORDNO=10&quot; &gt;', ''), '&lt;a href=&quot;https://www.supremecourt.ohio.gov/SCO/formerjustices/bios/allen.asp&quot; &gt;', ''), '&lt;/a&gt;', ''), '&lt;a href=&quot;http://blog.case.edu/archives/2012/03/30/celebrate_womenas_history_month_florence_e_allen&quot; &gt;', ''), '&lt;/em&gt;', ''), '&lt;em&gt;', ''), '&lt;/p&gt;', ''), '&lt;p&gt;', '')) "/>
          </xsl:element>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
  ...
  <xsl:template match="dc:identifier" mode="kent_allen">
