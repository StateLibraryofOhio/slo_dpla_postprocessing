...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Marysville Public Library</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Fairbanks Local School District Yearbooks</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="marysvillepl_p16802coll42"/>      <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:type"                  mode="marysvillepl_p16802coll42"/> <!-- create dcterms:type                                        -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative"      mode="odn"/>                     <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="marysvillepl_p16802coll42"/><!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:isVersionOf"      mode="marysvillepl_p16802coll42"/>
      <xsl:apply-templates select="dcterms:accessRights"     mode="marysvillepl_p16802coll42"/>


    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:type" mode="marysvillepl_p16802coll42"/>

  <xsl:template match="dcterms:isTypeOf" mode="marysvillepl_p16802coll42"/>

  <xsl:template match="dc:rights" mode="marysvillepl_p16802coll42"/>

  <xsl:template match="dcterms:isVersionOf" mode="marysvillepl_p16802coll42"/>

  <xsl:template match="dc:contributor" mode="marysvillepl_p16802coll42">
  ...
  <xsl:template match="dcterms:accessRights" mode="marysvillepl_p16802coll42">
    <xsl:for-each select="tokenize(., ';')">
