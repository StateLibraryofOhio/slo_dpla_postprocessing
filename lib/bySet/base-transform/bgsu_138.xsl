...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Craig Shipbuilding Company Glass Plate Negatives</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_138"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="odn"/>                     <!-- create edm:rights    and dc:rights                         -->
      <xsl:apply-templates select="dc:title"                 mode="bgsu_138"/>                <!-- create dcterms:title                                       -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_138"/>                <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="bgsu_138"/>

  <xsl:template match="dc:title" mode="bgsu_138">
    <xsl:element namespace="http://purl.org/dc/terms/" name="dcterms:title">
      <xsl:value-of select="normalize-space(replace(replace(., '&amp;nbsp; &lt;span style=&quot;text-decoration: underline;&quot;&gt;', ''), '&lt;/span&gt;', ''))"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="bgsu_138">
    <xsl:choose>
