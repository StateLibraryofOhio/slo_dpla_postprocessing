...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">J. Thomas Sebald Realty Company Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="midpointe_p16488coll2"/>   <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="midpointe_p16488coll2"/>   <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="midpointe_p16488coll2"/>   <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="midpointe_p16488coll2"/>

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:identifier" mode="midpointe_p16488coll2">
    <xsl:choose>
  ...
  <xsl:template match="dc:coverage" mode="midpointe_p16488coll2">
  ...
  <xsl:template match="dcterms:extent" mode="midpointe_p16488coll2">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll2">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
