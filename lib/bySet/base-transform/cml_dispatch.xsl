...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:apply-templates select="dc:contributor"           mode="cml_dispatch"/>            <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="cml_dispatch"/>            <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="cml_dispatch"/>            <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="odn"/>                     <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cml_dispatch"/>            <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_dispatch"/>            <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->

      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_dispatch"/>

      <xsl:apply-templates select="dcterms:issued"           mode="cml_dispatch"/>
      <xsl:apply-templates select="dc:coverage"              mode="cml_dispatch"/>
      <xsl:apply-templates select="dcterms:medium"           mode="cml_dispatch"/>
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_dispatch"/>
      <xsl:apply-templates select="dcterms:mediator"         mode="cml_dispatch"/>
      <xsl:apply-templates select="dcterms:references"       mode="cml_dispatch"/>
      <xsl:apply-templates select="dcterms:isVersionOf"      mode="cml_dispatch"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:issued" mode="cml_dispatch"/>
  <xsl:template match="dcterms:temporal" mode="cml_dispatch"/>
  <xsl:template match="dcterms:medium" mode="cml_dispatch"/>
  <xsl:template match="dcterms:isPartOf" mode="cml_dispatch"/>
  <xsl:template match="dcterms:provenance" mode="cml_dispatch"/>
  <xsl:template match="dc:rights" mode="cml_dispatch"/>
  <xsl:template match="dcterms:mediator" mode="cml_dispatch"/>
  <xsl:template match="dcterms:spatial" mode="cml_dispatch"/>
  <xsl:template match="dcterms:references" mode="cml_dispatch"/>
  <xsl:template match="dcterms:isVersionOf" mode="cml_dispatch"/>
  <xsl:template match="dc:publisher" mode="cml_dispatch"/>
  <xsl:template match="dc:contributor" mode="cml_dispatch"/>
  <xsl:template match="dcterms:extent" mode="cml_dispatch"/>
  <xsl:template match="dc:creator" mode="cml_dispatch">
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_dispatch">
    <xsl:for-each select="tokenize(., ';')">
