...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Cleveland Theater</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="clevepl_p16014coll24"/>    <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="clevepl_p16014coll24"/>    <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="clevepl_p16014coll24"/>    <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="clevepl_p16014coll24"/>    <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:medium"           mode="clevepl_p16014coll24"/>    <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="clevepl_p16014coll24"/>    <!-- unwanted (temporarily?); remove                            -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="clevepl_p16014coll24"/>    <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:accessRights"     mode="clevepl_p16014coll24"/>    <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="clevepl_p16014coll24"/>    <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:license"          mode="clevepl_p16014coll24"/>    <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:provenance"       mode="clevepl_p16014coll24"/>    <!-- unwanted; remove                                           -->


    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:spatial" mode="clevepl_p16014coll24"/>

  <xsl:template match="dcterms:accessRights" mode="clevepl_p16014coll24"/>

  <xsl:template match="dcterms:isReferencedBy" mode="clevepl_p16014coll24"/>

  <xsl:template match="dcterms:license" mode="clevepl_p16014coll24"/>

  <xsl:template match="dcterms:provenance" mode="clevepl_p16014coll24"/>

  <xsl:template match="dc:format" mode="clevepl_p16014coll24"/>


  <xsl:template match="dc:identifier" mode="clevepl_p16014coll24">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:medium" mode="clevepl_p16014coll24">
  ...
  <xsl:template match="dc:creator" mode="clevepl_p16014coll24">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:contributor" mode="clevepl_p16014coll24">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:date" mode="clevepl_p16014coll24">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
