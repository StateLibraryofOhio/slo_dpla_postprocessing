...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Kent State College of Podiatric Medicine Yearbooks</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_cpmyearbooks"/>       <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="kent_cpmyearbooks"/>       <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_cpmyearbooks"/>       <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_cpmyearbooks"/>       <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_cpmyearbooks"/>

  <xsl:template match="dc:format" mode="kent_cpmyearbooks"/>

  <xsl:template match="dc:identifier" mode="kent_cpmyearbooks">
    <xsl:if test="not(ends-with(., '/viewcontent'))">
  ...
  <xsl:template match="dc:description" mode="kent_cpmyearbooks">
    <xsl:choose>
