...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="odn_templates.xsl"/>
    ...
    <xsl:template match="//oai_dc:dc">
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Zhou Documents</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"       mode="kenyon_zhoudocs"/>        <!-- create edm:isShownAt                 -->
      <xsl:apply-templates select="dc:description"      mode="kenyon_zhoudocs"/>        <!-- create edm:preview                   -->
      ...
      <xsl:apply-templates select="dc:date.created"     mode="kenyon_zhoudocs"/>        <!-- create dc:date                          -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial.lat"  mode="kenyon_zhoudocs"/>   <!-- create dcterms:spatial                  -->
      ...
      <xsl:apply-templates select="dc:description.abstract"      mode="kenyon_zhoudocs"/>    <!-- create dcterms:description  -->
  ...
  <xsl:template match="dc:identifier" mode="kenyon_zhoudocs">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="kenyon_zhoudocs">
  ...
  <xsl:template match="dc:description.abstract" mode="kenyon_zhoudocs">
    <xsl:element namespace="http://purl.org/dc/terms/" name="dcterms:description">
      <xsl:value-of select="replace(replace(normalize-space(.), '&lt;[/]*em&gt;', ''), '&lt;[/]*p&gt;', '')"/>
    </xsl:element>
  </xsl:template>


  <xsl:template match="dc:date.created" mode="kenyon_zhoudocs">
