...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Cummins Civil War Letters</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="mrcpl_set7"/>              <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:type"                  mode="mrcpl_set7"/>              <!-- create dcterms:type                                        -->
      ...
      <xsl:apply-templates select="dc:relation"              mode="mrcpl_set7"/>              <!-- create dc:relation                                         -->
  ...
  <xsl:template match="dc:type" mode="mrcpl_set7">
    <xsl:if test="not(preceding-sibling::dc:type/text() = current()/text())">
  ...
  <xsl:template match="dc:relation" mode="mrcpl_set7">
    <xsl:element name="edm:preview" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="mrcpl_set7">
    <xsl:choose>
