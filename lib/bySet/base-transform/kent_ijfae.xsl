...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">International Journal of First Aid Education</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_ijfae"/>              <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="kent_ijfae"/>              <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_ijfae"/>              <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_ijfae"/>              <!-- uncertain; shunting to dc:relation for now                 -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:format" mode="kent_ijfae"/>

  <xsl:template match="dc:identifier" mode="kent_ijfae">
  ...
  <xsl:template match="dc:source" mode="kent_ijfae">
  ...
  <xsl:template match="dc:description" mode="kent_ijfae">
    <xsl:choose>
      <xsl:when test="ends-with(., 'thumbnail.jpg')">
        <xsl:element name="edm:preview" namespace="http://www.europeana.eu/schemas/edm/">
          http://oaks.kent.edu<xsl:value-of select="."/>
        </xsl:element>
      </xsl:when> 
      <xsl:when test="not(normalize-space(.)='')">
        <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="replace(replace(replace(replace(replace(replace(replace(., '&amp;nbsp;', ''), '&lt;/*span&gt;', ''), '&lt;/*em&gt;', ''), '&lt;/*li&gt;', ''), '&lt;/*ul&gt;', ''), '&lt;/*strong&gt;', ''), '&lt;/*p&gt;', '')"/>
        </xsl:element>
      </xsl:when> 
    </xsl:choose>
  </xsl:template>
