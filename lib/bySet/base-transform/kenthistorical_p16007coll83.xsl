...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Kent Historical Society and Museum Digital Collection</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Kent Ohio Oral History collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kenthistorical_p16007coll83"/> <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="kenthistorical_p16007coll83"/>  <!-- create dcterms:creator                                     -->
      ...
      <xsl:apply-templates select="dcterms:spatial"          mode="kenthistorical_p16007coll83"/>  <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="kenthistorical_p16007coll83"/>  <!-- do not create dcterms:contributor                           -->
      ...
      <xsl:apply-templates select="dcterms:license"          mode="kenthistorical_p16007coll83"/>  <!-- create edm:rights -->
      <xsl:apply-templates select="dc:source"                mode="kenthistorical_p16007coll83"/>
      <xsl:apply-templates select="dcterms:medium"           mode="kenthistorical_p16007coll83"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:contributor" mode="kenthistorical_p16007coll83"/>
  <xsl:template match="dc:source" mode="kenthistorical_p16007coll83"/>
  <xsl:template match="dcterms:medium" mode="kenthistorical_p16007coll83"/>


  <xsl:template match="dcterms:license" mode="kenthistorical_p16007coll83">
    <xsl:element name="edm:rights" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
	</xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:spatial" mode="kenthistorical_p16007coll83">
  ...
  <xsl:template match="dc:creator" mode="kenthistorical_p16007coll83">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:identifier" mode="kenthistorical_p16007coll83">
    <xsl:choose>
