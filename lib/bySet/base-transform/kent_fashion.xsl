...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Historical Fashion and Textile Books, Brochures, and Small Collections</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_fashion"/>            <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_fashion"/>            <!-- create edm:preview                                         -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_fashion"/>            <!-- create edm:isPartOf                                        -->


    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:description" mode="kent_fashion">
    <xsl:choose>
      ...
      <xsl:when test="contains(., 'Full scan of')">
        <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="replace(replace(replace(., '&amp;nbsp;', ' '), '&lt;/?em&gt;', ''), '&lt;/?p&gt;', '')"/>
        </xsl:element>
      </xsl:when>
    </xsl:choose>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_fashion">
    <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="kent_fashion">
