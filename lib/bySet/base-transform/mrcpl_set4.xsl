...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Mansfield / Richland County Public Library</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Mansfield-Richland County Photo Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="mrcpl_set4"/>              <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:relation"              mode="mrcpl_set4"/>              <!-- create edm:preview                                         -->
      ...
      <xsl:apply-templates select="dc:source"                mode="mrcpl_set4"/>              <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:coverage"              mode="mrcpl_set4"/>              <!-- create dcterms:spatial -->
    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:source" mode="mrcpl_set4"/>

  <xsl:template match="dc:relation" mode="mrcpl_set4">
    <xsl:element name="edm:preview" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:coverage" mode="mrcpl_set4">
  ...
  <xsl:template match="dc:identifier" mode="mrcpl_set4">
    <xsl:choose>
      <xsl:when test="contains(., '/items/show/') and starts-with(., 'http')">
