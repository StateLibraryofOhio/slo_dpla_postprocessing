...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Herbarium Sheets Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="wooster_herbarium"/>       <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="wooster_herbarium"/>       <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="wooster_herbarium"/>       <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dc:description"           mode="wooster_herbarium"/>       <!-- create edm:isShownAt                                       -->
      <xsl:apply-templates select="dc:description.abstract"  mode="wooster_herbarium"/>       <!-- create dcterms:description                                 -->
  ...
  <xsl:template match="dc:rights" mode="wooster_herbarium"><xsl:element name="edm:rights" namespace="http://www.europeana.eu/schemas/edm/">http<xsl:value-of select="normalize-space(substring-after(substring-before(., '/1.0/'), 'http'))" />/1.0/</xsl:element>
  </xsl:template>

  <xsl:template match="dc:date.created" mode="wooster_herbarium">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="substring-before(normalize-space(.),'T')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="wooster_herbarium">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="wooster_herbarium">
    <xsl:element name="edm:preview" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:description.abstract" mode="wooster_herbarium">
    <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="replace(replace(., '&lt;br /&gt;', ''), '&lt;[/]*p&gt;', '')"/>
    </xsl:element>
  </xsl:template>
