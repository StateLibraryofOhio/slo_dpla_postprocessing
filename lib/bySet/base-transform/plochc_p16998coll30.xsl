...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">             <!-- this may need to be changed if the server isn't running CONTENTdm -->
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Photographs and Prints</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="plochc_p16998coll30"/>     <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="plochc_p16998coll30"/>     <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="plochc_p16998coll30"/>     <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="plochc_p16998coll30"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="plochc_p16998coll30"/>     <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="plochc_p16998coll30"/>     <!-- unwanted; remove                                           -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="plochc_p16998coll30"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:relation"              mode="plochc_p16998coll30"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="odn"/>                     <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:apply-templates select="dcterms:rightsHolder"     mode="plochc_p16998coll30"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:temporal"         mode="odn"/>                     <!-- create dcterms:temporal                                    -->

      <xsl:apply-templates select="dcterms:medium"           mode="plochc_p16998coll30"/>     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:license"          mode="plochc_p16998coll30"/>     <!-- create dc:format                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template  match="dc:format" mode="plochc_p16998coll30"/>
  <xsl:template  match="dc:publisher" mode="plochc_p16998coll30"/>
  <xsl:template  match="dcterms:extent" mode="plochc_p16998coll30"/>
  <xsl:template  match="dcterms:license" mode="plochc_p16998coll30"/>
  <xsl:template  match="dcterms:rightsHolder" mode="plochc_p16998coll30"/>
  <xsl:template  match="dc:relation" mode="plochc_p16998coll30"/>
  ...
  <xsl:template match="dc:identifier" mode="plochc_p16998coll30">
    <xsl:choose>
  ...
  <xsl:template match="dc:date" mode="plochc_p16998coll30">
  ...
  <xsl:template match="dcterms:medium" mode="plochc_p16998coll30">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:creator" mode="plochc_p16998coll30">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:spatial" mode="plochc_p16998coll30">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
