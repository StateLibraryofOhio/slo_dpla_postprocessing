...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Nickel Weeklies</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_15"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:language"              mode="bgsu_15"/>                 <!-- create dcterms:language                                    -->
      ...
      <xsl:apply-templates select="dc:format"                mode="bgsu_15"/>                 <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="bgsu_15"/>                 <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="bgsu_15"/>                 <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dc:description"           mode="bgsu_15"/>                 <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:relation"              mode="bgsu_15"/>                 <!-- unwanted; remove                                           -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_15"/>                 <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:format" mode="bgsu_15"/>
  <xsl:template match="dc:relation" mode="bgsu_15"/>
  <xsl:template match="dc:source" mode="bgsu_15"/>

  <xsl:template match="dc:identifier" mode="bgsu_15">
    <xsl:choose>
  ...
  <xsl:template match="dc:language" mode="bgsu_15">
    <xsl:element name="dcterms:language" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(replace(., '[eE][nN]-[uU][sS]', 'English'))"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:spatial" mode="bgsu_15">
  ...
  <xsl:template match="dc:subject" mode="bgsu_15">
    <xsl:for-each select="tokenize(., '\|\|')">
  ...
  <xsl:template match="dc:description" mode="bgsu_15">
    <xsl:element namespace="http://purl.org/dc/terms/" name="dcterms:description">
      <xsl:value-of select="replace(replace(replace(replace(normalize-space(.), '&lt;[/]*em&gt;', ''), '&lt;[/]*p&gt;', ''), '&lt;br /&gt;', ' '), '&lt;[/]*strong&gt;', '')"/>
    </xsl:element>
  </xsl:template>
