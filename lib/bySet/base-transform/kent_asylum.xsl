...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Insane Asylum memoirs, 1842-1890</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_asylum"/>             <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_asylum"/>             <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_asylum"/>             <!-- unwanted; remove                                           -->
    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_asylum"/>

  <xsl:template match="dc:identifier" mode="kent_asylum">
  ...
  <xsl:template match="dc:description" mode="kent_asylum">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test="not(normalize-space(.)='')">
          <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
            <xsl:value-of select="normalize-space(replace(replace(replace(., '&lt;/*em&gt;', ' '), '&lt;/*span&gt;', ''), '&lt;/*p&gt;', ''))"/>
          </xsl:element>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
