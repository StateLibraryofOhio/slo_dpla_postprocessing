...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Taylorcraft (Alliance, Ohio)</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="rodman_taylor"/>           <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="rodman_taylor"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="rodman_taylor"/>           <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="rodman_taylor"/>           <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="rodman_taylor"/>           <!-- unwanted; remove                                           -->
      ...
      <xsl:apply-templates select="dcterms:license"          mode="rodman_taylor"/>           <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:contributor" mode="rodman_taylor"/>                                 <!-- unwanted; remove                                           -->
  <xsl:template match="dcterms:license" mode="rodman_taylor"/>                                <!-- unwanted; remove                                           -->

  <xsl:template match="dc:date" mode="rodman_taylor">
  ...
  <xsl:template match="dcterms:spatial" mode="rodman_taylor">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:identifier" mode="rodman_taylor">
    <xsl:choose>
  ...
  <xsl:template match="dc:format" mode="rodman_taylor">
    <xsl:for-each select="tokenize(., ';')">
