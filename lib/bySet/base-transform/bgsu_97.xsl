...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">             <!-- this may need to be changed if the server isn't running CONTENTdm -->
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">History 303 World War II Oral Histories</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_97"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_97"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:source"                mode="bgsu_97"/>                 <!-- unwanted; remove                                           -->
  ...
  <!-- unwanted; remove -->
  <xsl:template match="dc:source" mode="bgsu_97"/>
  <xsl:template match="dcterms:created" mode="bgsu_97"/>


  <!-- create the edm:isShownAt and edm:preview values -->
  <xsl:template match="dc:identifier" mode="bgsu_97">
    <xsl:choose>
