...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Columbus Chill</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="cml_columbuschill"/>       <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="cml_columbuschill"/>       <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="cml_columbuschill"/>       <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="cml_columbuschill"/>       <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="cml_columbuschill"/>       <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cml_columbuschill"/>       <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_columbuschill"/>       <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->

      <xsl:apply-templates select="dcterms:issued"           mode="cml_columbuschill"/>
      <xsl:apply-templates select="dc:coverage"              mode="cml_columbuschill"/>
      <xsl:apply-templates select="dcterms:medium"           mode="cml_columbuschill"/>
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_columbuschill"/>
      <xsl:apply-templates select="dcterms:mediator"         mode="cml_columbuschill"/>
      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_columbuschill"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:issued" mode="cml_columbuschill"/>
  <xsl:template match="dcterms:temporal" mode="cml_columbuschill"/>
  <xsl:template match="dcterms:medium" mode="cml_columbuschill"/>
  <xsl:template match="dcterms:provenance" mode="cml_columbuschill"/>
  <xsl:template match="dc:rights" mode="cml_columbuschill"/>
  <xsl:template match="dcterms:mediator" mode="cml_columbuschill"/>
  <xsl:template match="dcterms:spatial" mode="cml_columbuschill"/>
  <xsl:template match="dc:contributor" mode="cml_columbuschill"/>
  <xsl:template match="dcterms:isPartOf" mode="cml_columbuschill"/>

  <xsl:template match="dc:date" mode="cml_columbuschill">
  ...
  <xsl:template match="dc:creator" mode="cml_columbuschill">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:coverage" mode="cml_columbuschill">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_columbuschill">
    <xsl:for-each select="tokenize(., ';')">
