...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Bulmash Family Holocaust Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kenyon_bulmash"/>          <!-- create edm:isShownAt, edm:preview, and dcterms:identifier -->
      <xsl:apply-templates select="dc:rights"                mode="odn"/>                     <!-- create edm:rights    and dc:rights        -->
      <xsl:apply-templates select="dc:title"                 mode="kenyon_bulmash"/>          <!-- create dcterms:title                      -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kenyon_bulmash"/>          <!-- create edm:preview                        -->
      <xsl:apply-templates select="dc:description.abstract"  mode="kenyon_bulmash"/>          <!-- create edm:preview                        -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="kenyon_bulmash"/>          <!-- create dc:relation                        -->
  ...
  <xsl:template match="dc:title" mode="kenyon_bulmash">
  ...
  <xsl:template match="dc:date.created" mode="kenyon_bulmash">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="substring-before(., 'T')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="kenyon_bulmash">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="kenyon_bulmash">
