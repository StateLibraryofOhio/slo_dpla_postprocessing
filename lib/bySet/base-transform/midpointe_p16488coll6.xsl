...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="odn_templates.xsl"/>
    ...
    <xsl:template match="//oai_qdc:qualifieddc">
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Historic Book Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"       mode="midpointe_p16488coll6"/>         <!-- create edm:isShownAt and dcterms:identifier -->
      ...
      <xsl:apply-templates select="dc:language"         mode="midpointe_p16488coll6"/>      <!-- create dcterms:language                 -->
      <xsl:apply-templates select="dc:creator"          mode="midpointe_p16488coll6"/>      <!-- create dcterms:creator                  -->
      <xsl:apply-templates select="dc:date"             mode="midpointe_p16488coll6"/>      <!-- create dc:date                          -->
      <xsl:apply-templates select="dc:format"           mode="midpointe_p16488coll6"/>      <!-- create dc:format                        -->
      <xsl:copy-of         select="dcterms:spatial"     copy-namespaces="no"/>              <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:subject"          mode="midpointe_p16488coll6"/>      <!-- create dcterms:subject                  -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative" mode="odn"/>                        <!-- create dcterms:alternative  -->
      <xsl:apply-templates select="dc:contributor"      mode="midpointe_p16488coll6"/>      <!-- create dcterms:contributor  -->
      <xsl:apply-templates select="dc:description"      mode="odn"/>                        <!-- create dcterms:description  -->
      <xsl:apply-templates select="dcterms:extent"      mode="midpointe_p16488coll6"/>      <!-- create dcterms:extent       -->
                                                                                            <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"        mode="midpointe_p16488coll6"/>      <!-- create dcterms:publisher    -->
      <xsl:apply-templates select="dc:relation"         mode="midpointe_p16488coll6"/>      <!-- create dc:relation          -->
      <xsl:apply-templates select="dc:source"           mode="midpointe_p16488coll6"/>      <!-- create dc:relation          -->
  ...
  <xsl:template match="dc:identifier" mode="midpointe_p16488coll6">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:extent" mode="midpointe_p16488coll6">
  ...
  <xsl:template match="dc:publisher" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="not(normalize-space(.)='')">
  ...
  <xsl:template match="dc:contributor" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="not(normalize-space(.)='')">
  ...
  <xsl:template match="dc:date" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="not(normalize-space(.)='')">
  ...
  <xsl:template match="dc:creator" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="not(normalize-space(.)='')">
  ...
  <xsl:template match="dc:language" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="not(normalize-space(.)='')">
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != 'Image/TIFF' and normalize-space(.) != ''">
  ...
  <xsl:template match="dc:subject" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
        <xsl:element name="dcterms:subject" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="normalize-space(replace(., '\s*--\s*', '--'))"/>
        </xsl:element>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>

  <xsl:template match="dc:relation" mode="midpointe_p16488coll6">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:source" mode="midpointe_p16488coll6">
    <xsl:element name="dc:relation" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>
