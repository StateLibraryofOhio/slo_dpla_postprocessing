...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Barnett Hook Papers and Needlework</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="ohiou_p15808coll9"/>       <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="ohiou_p15808coll9"/>       <!-- create IIIF metadata                                       -->
      <xsl:apply-templates select="dc:publisher"             mode="odn"/>                     <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="ohiou_p15808coll9"/>       <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="ohiou_p15808coll9"/>       <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dcterms:medium"           mode="ohiou_p15808coll9"/>       <!-- frequently unused; remove by default                       -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:isReferencedBy" mode="ohiou_p15808coll9"/>
  ...
  <xsl:template match="dc:contributor" mode="ohiou_p15808coll9">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:creator" mode="ohiou_p15808coll9">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:spatial" mode="ohiou_p15808coll9">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
