...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Kent State Shootings: Oral Histories</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_omeka3"/>             <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="kent_omeka3"/>             <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="kent_omeka3"/>             <!-- create dcterms:creator                                     -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial"      mode="kent_omeka3"/>             <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="kent_omeka3"/>             <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_omeka3"/>             <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="odn"/>                     <!-- create dcterms:extent                                      -->
      <xsl:apply-templates select="dc:duration"              mode="kent_omeka3"/>             <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dc:relation.ispartof"     mode="kent_omeka3"/>             <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dc:repository"            mode="kent_omeka3"/>             <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="kent_omeka3"/>             <!-- create dcterms:temporal                                    -->
      <xsl:apply-templates select="dc:coverage.temporal"     mode="kent_omeka3"/>             <!-- create dcterms:temporal                                    -->

      <xsl:apply-templates select="dc:rights.uri"            mode="kent_omeka3"/>             <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:rights.uri" mode="kent_omeka3"/>

  <xsl:template match="dc:coverage.spatial" mode="kent_omeka3"/>

  <xsl:template match="dcterms:spatial" mode="kent_omeka3"/>

  <xsl:template match="dc:creator" mode="kent_omeka3">
  ...
  <xsl:template match="dc:coverage.temporal" mode="kent_omeka3">
   <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:duration" mode="kent_omeka3">
  ...
  <xsl:template match="dc:identifier" mode="kent_omeka3">
    <xsl:element name="edm:isShownAt" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:rights" mode="kent_omeka3">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="kent_omeka3">
