...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">The Pontifical College Josephinum (November 26, 1931-)</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:language"              mode="joseph_p17077coll3"/>      <!-- create dcterms:language                                    -->
      <xsl:apply-templates select="dc:creator"               mode="joseph_p17077coll3"/>      <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="joseph_p17077coll3"/>      <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="joseph_p17077coll3"/>      <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:source"                mode="joseph_p17077coll3"/>      <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="joseph_p17077coll3"/>


  <xsl:template match="dc:date" mode="joseph_p17077coll3">
  ...
  <xsl:template match="dc:language" mode="joseph_p17077coll3">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:spatial" mode="joseph_p17077coll3">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:creator" mode="joseph_p17077coll3">
