...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Gramm-Bernstein Motor Truck Company collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_93"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_93"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:source"                mode="bgsu_93"/>                 <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dcterms:created" mode="bgsu_93"/>
  <xsl:template match="dc:source" mode="bgsu_93"/>

  <xsl:template match="dc:identifier" mode="bgsu_93">
    <xsl:choose>
