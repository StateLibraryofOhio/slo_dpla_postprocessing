...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Grimshaw Brothers Silk Samples</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_grimshaw"/>           <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_grimshaw"/>           <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_grimshaw"/>           <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_grimshaw"/>   


<!--
  <xsl:template match="dc:description" mode="kent_grimshaw">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test="not(normalize-space(.)='')">
          <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
            <xsl:value-of select="normalize-space(replace(replace(replace(., '&lt;br/&gt;', '|'), '&lt;/a&gt;', ''), ': &lt;a href=.*&gt;', ': '))"/>
          </xsl:element>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
...
-->
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:if test="normalize-space(.) != ''">
            <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
              <xsl:value-of select="normalize-space(replace(replace(., '&lt;/a&gt;', ''), '&lt;a href.*&gt;', ''))"/>
            </xsl:element>
          </xsl:if>
        </xsl:otherwise>
      </xsl:choose>
  ...
  <xsl:template match="dc:identifier" mode="kent_grimshaw">
