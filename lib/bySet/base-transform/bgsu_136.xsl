...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Lake Carriers' Association Minutes and Scrapbooks</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_136"/>               <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="bgsu_136"/>                <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="odn"/>                    <!-- create IIIF metadata                                       -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_136"/>               <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:source" mode="bgsu_136"/>

  <xsl:template match="dc:rights" mode="bgsu_136">
    <xsl:choose>
  ...
  <xsl:template match="dc:identifier" mode="bgsu_136">
    <xsl:choose>
