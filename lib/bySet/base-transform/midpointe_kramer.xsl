...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Harold and Carolyn G. Kramer Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="midpointe_kramer"/>        <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="midpointe_kramer"/>        <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="midpointe_kramer"/>        <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="midpointe_kramer"/>        <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="midpointe_kramer"/>         <!-- to dcterms:spatial  -->
      <xsl:apply-templates select="dcterms:license"          mode="midpointe_kramer"/>         <!-- to dc:rights        -->

      <xsl:apply-templates select="dc:source"                mode="midpointe_kramer"/>         <!-- unwanted; remove -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="midpointe_kramer"/>

  <xsl:template match="dc:format" mode="midpointe_kramer">
  ...
  <xsl:template match="dc:coverage" mode="midpointe_kramer">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:license" mode="midpointe_kramer">
    <xsl:element namespace="http://purl.org/dc/elements/1.1/" name="dc:rights">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:extent" mode="midpointe_kramer">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:subject" mode="midpointe_kramer">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="midpointe_kramer">
    <xsl:choose>
