...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">             <!-- this may need to be changed if the server isn't running CONTENTdm -->
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">The Tomahawk - Madison Local School District High School Yearbooks</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:format"                mode="midpointe_p16488coll9"/>   <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:coverage"              mode="midpointe_p16488coll9"/>   <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:source"                mode="midpointe_p16488coll9"/>   <!-- discard dc:source                                          -->
  ...
  <xsl:template match="dc:source" mode="midpointe_p16488coll9"/>



  <xsl:template match="dc:format" mode="midpointe_p16488coll9">
  ...
  <xsl:template match="dc:coverage" mode="midpointe_p16488coll9">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
