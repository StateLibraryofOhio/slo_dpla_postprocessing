...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Columbus Free Press</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="cml_freepress"/>           <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="cml_freepress"/>           <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cml_freepress"/>           <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_freepress"/>           <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->

      <xsl:apply-templates select="dcterms:mediator"         mode="cml_freepress"/>
      <xsl:apply-templates select="dcterms:issued"           mode="cml_freepress"/>
      <xsl:apply-templates select="dcterms:medium"           mode="cml_freepress"/>
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_freepress"/>
      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_freepress"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:issued" mode="cml_freepress"/>

  <xsl:template match="dcterms:temporal" mode="cml_freepress"/>

  <xsl:template match="dcterms:medium" mode="cml_freepress"/>

  <xsl:template match="dcterms:isPartOf" mode="cml_freepress"/>

  <xsl:template match="dcterms:provenance" mode="cml_freepress"/>

  <xsl:template match="dc:rights" mode="cml_freepress"/>

  <xsl:template match="dcterms:mediator" mode="cml_freepress"/>

  <xsl:template match="dcterms:accessRights" mode="cml_freepress">
  ...
  <xsl:template match="dc:identifier" mode="cml_freepress">
    <xsl:choose>
  ...
  <xsl:template match="dc:date" mode="cml_freepress">
    <xsl:for-each select="tokenize(., ';')">
