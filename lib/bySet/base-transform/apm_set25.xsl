...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">American Motors Corporation</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="apm_set25"/>               <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="apm_set25"/>               <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="apm_set25"/>               <!-- create dc:date                                             -->
  ...
  <xsl:template match="dc:date" mode="apm_set25">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="apm_set25">
    <xsl:choose>
  ...
  <xsl:template match="dc:rights" mode="apm_set25">
    <xsl:choose>
