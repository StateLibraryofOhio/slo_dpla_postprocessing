...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Harlem Township</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="delawaremem_harlem"/>   <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="delawaremem_harlem"/>   <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="delawaremem_harlem"/>      <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dc:subject"               mode="delawaremem_harlem"/>   <!-- create dcterms:subject                                     -->
      <xsl:apply-templates select="dc:type"                  mode="delawaremem_harlem"/>   <!-- create dcterms:type                                        -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative"      mode="odn"/>                     <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="delawaremem_harlem"/>      <!-- create dcterms:contributor                                 -->
  ...
  <xsl:template match="dc:date" mode="delawaremem_harlem">
  ...
  <xsl:template match="dc:contributor" mode="delawaremem_harlem">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:subject" mode="delawaremem_harlem">
    <xsl:for-each select="tokenize(., '&#xA;')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="delawaremem_harlem">
    <xsl:choose>
  ...
  <xsl:template match="dc:type" mode="delawaremem_harlem">
    <xsl:for-each select="tokenize(., '&#xA;')">
        <xsl:choose>
  ...
  <xsl:template match="dc:rights" mode="delawaremem_harlem">
    <xsl:for-each select="tokenize(., '&#xA;')">
