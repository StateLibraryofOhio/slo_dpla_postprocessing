...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Carol Luper Newsmakers</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="cml_luper"/>               <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="cml_luper"/>               <!-- create dcterms:creator                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="cml_luper"/>               <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_luper"/>               <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->
      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_luper"/>
      <xsl:apply-templates select="dc:coverage"              mode="cml_luper"/>
      <xsl:apply-templates select="dcterms:medium"           mode="cml_luper"/>
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_luper"/>
      <xsl:apply-templates select="dcterms:mediator"         mode="cml_luper"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:rights" mode="cml_luper"/>
  <xsl:template match="dc:contributor" mode="cml_luper"/>
  <xsl:template match="dcterms:temporal" mode="cml_luper"/>
  <xsl:template match="dcterms:medium" mode="cml_luper"/>
  <xsl:template match="dcterms:provenance" mode="cml_luper"/>
  <xsl:template match="dcterms:mediator" mode="cml_luper"/>

  <xsl:template match="dc:coverage" mode="cml_luper">
  ...
  <xsl:template match="dc:creator" mode="cml_luper">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_luper">
    <xsl:for-each select="tokenize(., ';')">
