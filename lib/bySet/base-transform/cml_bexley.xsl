...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Bexley Public Library</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Bexley Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="cml_bexley"/>              <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="cml_bexley"/>              <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dcterms:spatial"          mode="cml_bexley"/>              <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="odn"/>                     <!-- create dcterms:subject                                     -->
      <xsl:apply-templates select="dc:type"                  mode="cml_bexley"/>                     <!-- create dcterms:type                                        -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="cml_bexley"/>              <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cml_bexley"/>              <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_bexley"/>              <!-- create dcterms:temporal                                    -->

      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_bexley"/>              <!-- create edm:rights                                          -->

      <xsl:apply-templates select="dcterms:provenance"       mode="cml_bexley"/>              <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isVersionOf"      mode="cml_bexley"/>              <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:medium"           mode="cml_bexley"/>              <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:coverage"              mode="cml_bexley"/> 

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:rights" mode="cml_bexley"/>
  <xsl:template match="dc:source" mode="cml_bexley"/>
  <xsl:template match="dcterms:extent" mode="cml_bexley"/>
  <xsl:template match="dcterms:isPartOf" mode="cml_bexley"/>
  <xsl:template match="dcterms:isVersionOf" mode="cml_bexley"/>
  <xsl:template match="dcterms:provenance" mode="cml_bexley"/>
  <xsl:template match="dcterms:temporal" mode="cml_bexley"/>
  <xsl:template match="dc:type" mode="cml_bexley"/>
  <xsl:template match="dcterms:medium" mode="cml_bexley"/>

  <xsl:template match="dcterms:spatial" mode="cml_bexley">
  ...
  <xsl:template match="dc:coverage" mode="cml_bexley">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="(normalize-space(.) != '')">
        <xsl:element name="dcterms:spatial" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="normalize-space(.)"/>
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_bexley">
  ...
  <xsl:template match="dc:identifier" mode="cml_bexley">
    <xsl:choose>
