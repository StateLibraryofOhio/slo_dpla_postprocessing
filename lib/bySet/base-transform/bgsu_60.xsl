...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Richard F. Blinn diary</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_60"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_60"/>                 <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="bgsu_60"/>                 <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_60"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="bgsu_60"/>                 <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dc:source" mode="bgsu_60"/>

  <xsl:template match="dcterms:isReferencedBy" mode="bgsu_60"/>

  <xsl:template match="dc:identifier" mode="bgsu_60">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:created" mode="bgsu_60">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:format" mode="bgsu_60">
