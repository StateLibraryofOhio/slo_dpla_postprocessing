...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Everyone's Story Matters</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_ksuscp"/>             <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- unwanted; remove                                           -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_ksuscp"/>             <!-- has thumbnail URL; create edm:preview                      -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_ksuscp"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_ksuscp"/>

  <xsl:template match="dc:description" mode="kent_ksuscp">
    <xsl:choose>
      <xsl:when test="contains(., 'amazonaws.com/ksuscp/')">
  ...
  <xsl:template match="dc:identifier" mode="kent_ksuscp">
    <xsl:if test="contains(., 'kent.edu/ksuscp/')">
