...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">German Farm Histories</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="cml_germanfarm"/>          <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:language"              mode="cml_germanfarm"/>          <!-- create dcterms:language - unwanted, remove                 -->
      <xsl:apply-templates select="dc:creator"               mode="odn"/>                     <!-- create dcterms:creator - unwanted, remove                  -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cml_germanfarm"/>          <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dcterms:medium"           mode="cml_germanfarm"/>          <!-- unwanted, remove                                           -->
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_germanfarm"/>          <!-- unwanted, remove                                           -->
      <xsl:apply-templates select="dc:source"                mode="cml_germanfarm"/>          <!-- unwanted, remove                                           -->
      <xsl:apply-templates select="dc:type"                  mode="cml_germanfarm"/>          <!-- unwanted, remove                                           -->

      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_germanfarm"/>          <!-- create edm:rights                                          -->
      <xsl:apply-templates select="dc:coverage"              mode="cml_germanfarm"/> 

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:coverage" mode="cml_germanfarm">
  ...
  <xsl:template match="dc:language" mode="cml_germanfarm">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
        <xsl:choose>
          <xsl:when test="normalize-space(.)='ENG'">
          ...
          <xsl:when test="normalize-space(.)='GER'">
            <xsl:element namespace="http://purl.org/dc/terms/" name="dcterms:language">
  ...
  <xsl:template match="dc:type" mode="cml_germanfarm"/>

  <xsl:template match="dcterms:medium" mode="cml_germanfarm"/>

  <xsl:template match="dcterms:provenance" mode="cml_germanfarm"/>

  <xsl:template match="dc:source" mode="cml_germanfarm"/>

  <xsl:template match="dcterms:isPartOf" mode="cml_germanfarm"/>
  ...
  <xsl:template match="dc:identifier" mode="cml_germanfarm">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_germanfarm">
    <xsl:for-each select="tokenize(., ';')">
