...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Toledo Lucas County Public Library Auditor's Images</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:copy-of         select="dc:format"                copy-namespaces="no"/>           <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="tlcpl_p16007coll88"/>      <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:description"           mode="tlcpl_p16007coll88"/>      <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dcterms:medium"           mode="tlcpl_p16007coll88"/>      <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:created"          mode="tlcpl_p16007coll88"/>      <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:coverage"              mode="tlcpl_p16007coll88"/>      <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:license"          mode="tlcpl_p16007coll88"/>      <!-- create edm:rights                                          -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:created" mode="tlcpl_p16007coll88"/>
  <xsl:template match="dc:coverage" mode="tlcpl_p16007coll88"/>

  <xsl:template match="dcterms:license" mode="tlcpl_p16007coll88">
  ...
  <xsl:template match="dcterms:spatial" mode="tlcpl_p16007coll88">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dcterms:medium" mode="tlcpl_p16007coll88">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:description" mode="tlcpl_p16007coll88">
    <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>

    <xsl:for-each select="tokenize(replace(replace(., 'This image was produced by the Auditor''s Office in Lucas County, Ohio for tax assessment purposes.  Associated dates are approximate.  Descriptive terms related to this photograph include:', ''), '\|', ';'), ';')">  
      <xsl:choose>
        <xsl:when test="not(matches(normalize-space(.), '^[0-9]')) and not(matches(normalize-space(.), '^$'))">
