...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Lorain Public Library System</xsl:element>     <!-- creaataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Lorain Tornado Centennial:  Reinhart Collections</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="lorainpl_50"/>             <!-- create edm:isShownAt, edm:preview, and dcterms:id  -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="odn"/>                     <!-- create dcterms:extent                              -->
                                                                                              <!-- dcterms:identifier is created above as part of thShownAt transform -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="odn"/>                     <!-- create dc:relation                                 -->
                                                                                              <!-- dc:rights is created above as part of the edm:rigsform -->
  ...
  <xsl:template match="dc:identifier" mode="lorainpl_50">
