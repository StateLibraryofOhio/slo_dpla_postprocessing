...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:apply-templates select="dc:identifier"            mode="kent_tglibrary"/>          <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="kent_tglibrary"/>

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:coverage" mode="kent_tglibrary"/>

  <xsl:template match="dc:identifier" mode="kent_tglibrary">
    <xsl:element name="edm:isShownAt" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:description" mode="kent_tglibrary">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:if test="not(normalize-space(.)='')">
          <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
            <xsl:value-of select="normalize-space(replace(replace(., '&lt;/?(br|table|tbody|tr|td|p|br /)&gt;', ' '), '&amp;nbsp;', ''))"/>
          </xsl:element>
        </xsl:if>
      </xsl:otherwise>
    </xsl:choose>
