...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Ohio History Journal</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_ohiohistory"/>        <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_ohiohistory"/>        <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_ohiohistory"/>        <!-- unwanted; remove                            -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="kent_ohiohistory"/>

  <xsl:template match="dc:description" mode="kent_ohiohistory">
    <xsl:choose>
      ...
      <xsl:otherwise>
        <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="replace(replace(replace(replace(replace(replace(replace(replace(., '&lt;/?h[12345]&gt;', ''), '&lt;/?strong&gt;', ''), '&amp;nbsp;', ''), '&lt;/?p&gt;', ''), '&lt;br /&gt;', ''), '&lt;/?ul&gt;', ''), '&lt;/?em&gt;', ''), '&lt;/?li&gt;' ,'')"/>
        </xsl:element>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="kent_ohiohistory">
