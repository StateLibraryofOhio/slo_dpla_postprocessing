...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Case Western Reserve University</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Cleveland Play House Production Photographs 1986-1987</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="cwru_productionphotos1986"/><!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:identifier.thumbnail"  mode="cwru_productionphotos"/>    <!-- get thumbnail      -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:identifier.thumbnail" mode="cwru_productionphotos">
  ...
  <xsl:template match="dc:identifier" mode="cwru_productionphotos1986">
    <xsl:choose>
      <xsl:when test="starts-with(., 'http://hdl.handle.net') or starts-with(., 'https://hdl.handle.net')">
