...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Department of Ethnic Studies records</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_51"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="bgsu_51"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:source"                mode="bgsu_51"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_51"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:references"       mode="bgsu_51"/>                 <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:coverage" mode="bgsu_51"/>

  <xsl:template match="dc:source" mode="bgsu_51"/>

  <xsl:template match="dcterms:created" mode="bgsu_51"/>

  <xsl:template match="dcterms:references" mode="bgsu_51"/>

  <xsl:template match="dc:identifier" mode="bgsu_51">
    <xsl:choose>
  ...
  <xsl:template match="dc:format" mode="bgsu_51">
    <xsl:if test="normalize-space(.) != 'application/pdf' and normalize-space(.) != 'video/mp4' and normalize-space(.) != 'image/jpeg'">
