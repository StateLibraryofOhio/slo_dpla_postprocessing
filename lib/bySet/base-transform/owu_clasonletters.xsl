...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:apply-templates select="dc:rights"                mode="owu_clasonletters"/>       <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dcterms:spatial"          mode="owu_clasonletters"/>       <!-- create dcterms:spatial                                     -->
  ...
  <xsl:template match="dcterms:spatial" mode="owu_clasonletters">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:rights" mode="owu_clasonletters">
    <xsl:for-each select="tokenize(., ';')">
        <xsl:if test="(normalize-space(.) != '') and (starts-with(normalize-space(replace(., 'Public domain ', '')), 'http://') and ends-with(normalize-space(.), '/'))" >
            <xsl:choose>
              <xsl:when test="starts-with(normalize-space(replace(., 'Public domain ', '')), 'http://rightsstatements.org/')">
                            <xsl:if test="index-of($rightsStatementsOrgList, normalize-space(replace(., 'Public domain ', '')))">
                                <xsl:element name="edm:rights" namespace="http://www.europeana.eu/schemas/edm/">
                                    <xsl:value-of select="normalize-space(replace(., 'Public domain ', ''))"/>
                                </xsl:element>
                            </xsl:if>
              </xsl:when>
              <xsl:otherwise>
