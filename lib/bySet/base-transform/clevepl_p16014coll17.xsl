...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Music at Cleveland Public Library</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="clevepl_p16014coll17"/>    <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="clevepl_p16014coll17"/>    <!-- create dcterms:creator                                     -->
      <xsl:copy-of         select="dc:date"                  copy-namespaces="no"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="clevepl_p16014coll17"/>    <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:medium"           mode="clevepl_p16014coll17"/>    <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="clevepl_p16014coll17"/>    <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="clevepl_p16014coll17"/>    <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="clevepl_p16014coll17"/>    <!-- create dcterms:publisher                                   -->
      ...
      <xsl:apply-templates select="dcterms:rightsHolder"     mode="clevepl_p16014coll17"/>    <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="odn"/>                     <!-- create dcterms:temporal                                    -->

      <xsl:apply-templates select="dcterms:accessRights"     mode="clevepl_p16014coll17"/>    <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:license"          mode="clevepl_p16014coll17"/>    <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:provenance"       mode="clevepl_p16014coll17"/>    <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:identifier" mode="clevepl_p16014coll17">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:medium" mode="clevepl_p16014coll17">
  ...
  <xsl:template match="dc:creator" mode="clevepl_p16014coll17">
  ...
  <xsl:template match="dcterms:spatial" mode="clevepl_p16014coll17">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:rightsHolder" mode="clevepl_p16014coll17"/>

  <xsl:template match="dcterms:provenance" mode="clevepl_p16014coll17"/>

  <xsl:template match="dcterms:license" mode="clevepl_p16014coll17"/>

  <xsl:template match="dcterms:accessRights" mode="clevepl_p16014coll17"/>

  <xsl:template match="dc:format" mode="clevepl_p16014coll17"/>

  <xsl:template match="dc:contributor" mode="clevepl_p16014coll17">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:publisher" mode="clevepl_p16014coll17">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
