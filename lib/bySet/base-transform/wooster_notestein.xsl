...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">English Historical Library of Wallace Notestein</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="wooster_notestein"/>       <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="wooster_notestein"/>       <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial"      mode="wooster_notestein"/>       <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:description"           mode="wooster_notestein"/>       <!-- create edm:preview                                         -->
      <xsl:apply-templates select="dc:description.abstract"  mode="wooster_notestein"/>       <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="wooster_notestein"/>       <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="wooster_notestein"/>

  <xsl:template match="dc:date.created" mode="wooster_notestein">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="substring-before(normalize-space(.),'T')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:coverage.spatial" mode="wooster_notestein">
    <xsl:element name="dcterms:spatial" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:description.abstract" mode="wooster_notestein">
  ...
  <xsl:template match="dc:description" mode="wooster_notestein">
    <xsl:element name="edm:preview" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="wooster_notestein">
