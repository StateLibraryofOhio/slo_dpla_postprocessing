...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Willoughby-Eastlake Public Library</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="clevepl_p16014coll23"/>    <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:format"                mode="clevepl_p16014coll23"/>    <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:medium"           mode="clevepl_p16014coll23"/>    <!-- create dc:format                          -->
      <xsl:apply-templates select="dcterms:spatial"          mode="clevepl_p16014coll23"/>    <!-- create dcterms:spatial                    -->
      ...
      <xsl:apply-templates select="dcterms:rightsHolder"     mode="clevepl_p16014coll23"/>    <!-- create dcterms:rightsHolder               -->
      <xsl:apply-templates select="dcterms:accessRights"     mode="clevepl_p16014coll23"/>    <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:temporal"         mode="odn"/>                     <!-- create dcterms:temporal                   -->

      <xsl:apply-templates select="dcterms:license"          mode="clevepl_p16014coll23"/>    <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:provenance"       mode="clevepl_p16014coll23"/>    <!-- unwanted; remove                          -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:accessRights" mode="clevepl_p16014coll23"/>

  <xsl:template match="dcterms:medium" mode="clevepl_p16014coll23">
    <xsl:element name="dc:format" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:spatial" mode="clevepl_p16014coll23">
  ...
  <xsl:template match="dcterms:rightsHolder" mode="clevepl_p16014coll23"/>

  <xsl:template match="dcterms:license" mode="clevepl_p16014coll23"/>

  <xsl:template match="dcterms:provenance" mode="clevepl_p16014coll23"/>

  <xsl:template match="dc:format" mode="clevepl_p16014coll23"/>

  <xsl:template match="dc:rights" mode="clevepl_p16014coll23">
