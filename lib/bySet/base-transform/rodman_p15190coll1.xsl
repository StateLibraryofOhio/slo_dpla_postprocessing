...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Alliance, Ohio Library</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="rodman_p15190coll1"/>      <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="rodman_p15190coll1"/>      <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="rodman_p15190coll1"/>      <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="rodman_p15190coll1"/>      <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:license"          mode="rodman_p15190coll1"/>      <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:contributor" mode="rodman_p15190coll1"/>                                 <!-- unwanted; remove                                           -->
  <xsl:template match="dcterms:license" mode="rodman_p15190coll1"/>                                <!-- unwanted; remove                                           -->

  <xsl:template match="dc:creator" mode="rodman_p15190coll1">
  ...
  <xsl:template match="dc:date" mode="rodman_p15190coll1">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:identifier" mode="rodman_p15190coll1">
    <xsl:choose>
