...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Lloyd E. Moore Photograph Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="ohiou_lloydmoore"/>        <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="ohiou_lloydmoore"/>        <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="ohiou_lloydmoore"/>        <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="ohiou_lloydmoore"/>        <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="ohiou_lloydmoore"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:isPartOf" mode="ohiou_lloydmoore"/>
  ...
  <xsl:template match="dcterms:spatial" mode="ohiou_lloydmoore">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:format" mode="ohiou_lloydmoore">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
