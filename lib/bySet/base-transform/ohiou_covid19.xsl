...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">COVID-19-Related Student Experiences and Observations</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="ohiou_covid19"/>           <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="ohiou_covid19"/>           <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="ohiou_covid19"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="ohiou_covid19"/>           <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="ohiou_covid19"/>           <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dcterms:provenance"       mode="ohiou_covid19"/>           <!-- create dcterms:temporal                                    -->
 
    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:provenance" mode="ohiou_covid19"/> 

  <xsl:template match="dc:date" mode="ohiou_covid19">
  ...
  <xsl:template match="dc:format" mode="ohiou_covid19">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:creator" mode="ohiou_covid19">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:spatial" mode="ohiou_covid19">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="ohiou_covid19">
    <xsl:choose>
