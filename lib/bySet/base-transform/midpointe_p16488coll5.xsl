...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="odn_templates.xsl"/>
    ...
    <xsl:template match="//oai_qdc:qualifieddc">
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Historical Butler County Atlases</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">MidPointe Library System</xsl:element>
      ...
      <xsl:apply-templates select="dc:creator"          mode="midpointe_p16488coll5"/>   <!-- create dcterms:creator                  -->
      <xsl:copy-of         select="dc:date"             copy-namespaces="no"/>           <!-- create dc:date                          -->
      <xsl:apply-templates select="dc:format"           mode="midpointe_p16488coll5"/>   <!-- create dc:format                        -->
      <xsl:copy-of         select="dcterms:spatial"     copy-namespaces="no"/>           <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:coverage"         mode="midpointe_p16488coll5"/>   <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:subject"          mode="midpointe_p16488coll5"/>   <!-- create dcterms:subject                  -->
      ...
      <xsl:apply-templates select="dcterms:extent"      mode="midpointe_p16488coll5"/>   <!-- create dcterms:extent       -->
      ...
      <xsl:apply-templates select="dc:source"           mode="midpointe_p16488coll5"/>   <!-- create dc:relation          -->
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll5">
  ...
  <xsl:template match="dc:creator" mode="midpointe_p16488coll5">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <!-- CONTENTdm passes multiple subjects separated by semicolons; split them out and copy them to dcterms:subject -->
  <xsl:template match="dc:subject" mode="midpointe_p16488coll5">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dcterms:extent" mode="midpointe_p16488coll5">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test=". != ''">
  ...
  <xsl:template match="dc:source" mode="midpointe_p16488coll5">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
   ...
   <xsl:template match="dc:coverage" mode="midpointe_p16488coll5">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
