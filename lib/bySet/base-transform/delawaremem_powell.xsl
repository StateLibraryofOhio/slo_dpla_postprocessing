...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Powell</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="delawaremem_powell"/>   <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="delawaremem_powell"/>   <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:subject"               mode="delawaremem_powell"/>      <!-- create dcterms:subject                                     -->
      <xsl:apply-templates select="dc:type"                  mode="delawaremem_powell"/>   <!-- create dcterms:type                                        -->
  ...
  <xsl:template match="dc:subject" mode="delawaremem_powell">
  ...
  <xsl:template match="dc:identifier" mode="delawaremem_powell">
    <xsl:choose>
  ...
  <xsl:template match="dc:type" mode="delawaremem_powell">
    <xsl:for-each select=".">
      <xsl:for-each select="tokenize(., '&#xA;')">
        <xsl:choose>
  ...
  <xsl:template match="dc:rights" mode="delawaremem_powell">
    <xsl:for-each select="tokenize(., '&#xA;')">
