...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Peter L. Goss Photograph Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:format"                mode="ohiou_peterlgoss"/>        <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="ohiou_peterlgoss"/>        <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="ohiou_peterlgoss"/>        <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="ohiou_peterlgoss"/>        <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="ohiou_peterlgoss"/>        <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->

      <xsl:apply-templates select="dcterms:created"          mode="ohiou_peterlgoss"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:temporal" mode="ohiou_peterlgoss"/>

  <xsl:template match="dc:contributor" mode="ohiou_peterlgoss"/>

  <xsl:template match="dcterms:isPartOf" mode="ohiou_peterlgoss"/>

  <xsl:template match="dcterms:created" mode="ohiou_peterlgoss">
  ...
  <xsl:template match="dc:format" mode="ohiou_peterlgoss">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:spatial" mode="ohiou_peterlgoss">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
