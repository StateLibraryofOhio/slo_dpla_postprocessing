...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Disabled American Veterans</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="plochc_p16998coll78"/>     <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="plochc_p16998coll78"/>     <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="plochc_p16998coll78"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:medium"           mode="plochc_p16998coll78"/>     <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="plochc_p16998coll78"/>     <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="plochc_p16998coll78"/>     <!-- unwanted; remove                                           -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="odn"/>                     <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="plochc_p16998coll78"/>     <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="plochc_p16998coll78"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:extent"           mode="plochc_p16998coll78"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:license"          mode="plochc_p16998coll78"/>     <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:rightsHolder"     mode="plochc_p16998coll78"/>     <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:format" mode="plochc_p16998coll78"/>

  <xsl:template match="dc:publisher" mode="plochc_p16998coll78"/>

  <xsl:template match="dcterms:extent" mode="plochc_p16998coll78"/>

  <xsl:template match="dcterms:license" mode="plochc_p16998coll78"/>

  <xsl:template match="dcterms:rightsHolder" mode="plochc_p16998coll78"/>

  <xsl:template match="dc:date" mode="plochc_p16998coll78">
  ...
  <xsl:template match="dcterms:medium" mode="plochc_p16998coll78">
    <xsl:element name="dc:format" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:contributor" mode="plochc_p16998coll78">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:relation" mode="plochc_p16998coll78"> 
  ...
  <xsl:template match="dc:identifier" mode="plochc_p16998coll78">
    <xsl:choose>
