...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">State Library of Ohio State Hospitals</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="ohmem_p267401cdi"/>        <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dcterms:medium"           mode="ohmem_p267401cdi"/>        <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dcterms:temporal"         mode="ohmem_p267401cdi"/>        <!-- create dc:date                                             -->

      <xsl:apply-templates select="dcterms:isReplacedBy"     mode="ohmem_p267401cdi"/>        <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="ohmem_p267401cdi">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:temporal" mode="ohmem_p267401cdi">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:medium" mode="ohmem_p267401cdi">
    <xsl:element name="dc:format" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:relation" mode="ohmem_p267401cdi"/>

  <xsl:template match="dcterms:isReplacedBy" mode="ohmem_p267401cdi"/>
