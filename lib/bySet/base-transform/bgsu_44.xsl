...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Sheet Music Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_44"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="bgsu_44"/>                 <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="bgsu_44"/>                 <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="bgsu_44"/>                 <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="bgsu_44"/>                 <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="bgsu_44"/>                 <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dcterms:references"       mode="bgsu_44"/>                 <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:references" mode="bgsu_44"/>
  <xsl:template match="dcterms:isPartOf" mode="bgsu_44"/>

  <xsl:template match="dc:contributor" mode="bgsu_44">
  ...
  <xsl:template match="dc:subject" mode="bgsu_44">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:format" mode="bgsu_44">
  ...
  <xsl:template match="dc:identifier" mode="bgsu_44">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="bgsu_44">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
        <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="normalize-space(replace(replace(., 'Capition title', 'Caption title'), 'Capion title', 'Caption title'))"/>
        </xsl:element>
      </xsl:if>
    </xsl:for-each>
  </xsl:template>
