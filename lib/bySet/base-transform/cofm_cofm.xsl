...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Central Ohio Fire Museum</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Central Ohio Fire Museum</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="cofm_cofm"/>               <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="cofm_cofm"/>               <!-- create dcterms:creator                                     -->
      ...
      <xsl:apply-templates select="dcterms:spatial"          mode="cofm_cofm"/>               <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="cofm_cofm"/>
      <xsl:apply-templates select="dcterms:accessRights"     mode="cofm_cofm"/>

    </oai_qdc:qualifieddc>

  </xsl:template>

  <xsl:template match="dc:rights" mode="cofm_cofm"/>  <!-- unwanted; their rights are in dcterms:accessRights -->

  <xsl:template match="dcterms:issued" mode="cofm_cofm"/>

  <xsl:template match="dcterms:temporal" mode="cofm_cofm"/>

  <xsl:template match="dcterms:spatial" mode="cofm_cofm"/>

  <xsl:template match="dcterms:medium" mode="cofm_cofm"/>

  <xsl:template match="dcterms:provenance" mode="cofm_cofm"/>

  <xsl:template match="dcterms:mediator" mode="cofm_cofm"/>

  <xsl:template match="dcterms:references" mode="cofm_cofm"/>

  <xsl:template match="dcterms:isVersionOf" mode="cofm_cofm"/>

  <xsl:template match="dc:coverage" mode="cofm_cofm">
  ...
  <xsl:template match="dc:creator" mode="cofm_cofm">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:accessRights" mode="cofm_cofm">
    <xsl:for-each select="tokenize(., ';')">
