...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Columbus Yearbook Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="cml_yearbook"/>            <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:language"              mode="cml_yearbook"/>            <!-- create dcterms:language  - uneanted, remove                -->
      <xsl:apply-templates select="dc:creator"               mode="odn"/>                     <!-- create dcterms:creator       -->
      <xsl:apply-templates select="dc:date"                  mode="cml_yearbook"/>            <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="cml_yearbook"/>            <!-- create dcterms:spatial  - unwanted, remove                 -->
      <xsl:apply-templates select="dc:subject"               mode="odn"/>                     <!-- create dcterms:subject                                     -->
      <xsl:apply-templates select="dc:type"                  mode="cml_yearbook"/>            <!-- create dcterms:type  - unwanted, remove                    -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative"      mode="odn"/>                     <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="cml_yearbook"/>            <!-- create dcterms:contributor  - unwanted, remove             -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="cml_yearbook"/>            <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dcterms:accrualMethod"    mode="cml_yearbook"/>            <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:coverage"              mode="cml_yearbook"/>            <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:source"                mode="cml_yearbook"/>            <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_yearbook"/>            <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:medium"           mode="cml_yearbook"/>            <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_yearbook"/>            <!-- map to edm:rights                                          -->

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:contributor" mode="cml_yearbook"/>

  <xsl:template match="dc:publisher" mode="cml_yearbook"/>

  <xsl:template match="dcterms:temporal" mode="cml_yearbook"/>

  <xsl:template match="dcterms:accrualMethod" mode="cml_yearbook"/>

  <xsl:template match="dc:source" mode="cml_yearbook"/>

  <xsl:template match="dcterms:provenance" mode="cml_yearbook"/>

  <xsl:template match="dc:language" mode="cml_yearbook"/>

  <xsl:template match="dc:type" mode="cml_yearbook"/>
  ...
  <xsl:template match="dc:coverage" mode="cml_yearbook">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="(normalize-space(.) != '')">
        <xsl:element name="dcterms:spatial" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="normalize-space(.)"/>
  ...
  <xsl:template match="dc:date" mode="cml_yearbook">
  ...
  <xsl:template match="dc:identifier" mode="cml_yearbook">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_yearbook">
    <xsl:for-each select="tokenize(., ';')">
