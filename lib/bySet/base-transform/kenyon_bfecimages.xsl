...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Brown Family Environmental Center Historic Photographs</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kenyon_bfecimages"/>       <!-- create edm:isShownAt                      -->
      <xsl:apply-templates select="dc:description"           mode="kenyon_bfecimages"/>       <!-- create edm:preview                        -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="kenyon_bfecimages"/>       <!-- create dc:date                            -->
      ...
      <xsl:apply-templates select="dc:description.abstract"  mode="kenyon_bfecimages"/>       <!-- create dcterms:description                -->
  ...
  <xsl:template match="dc:date.created" mode="kenyon_bfecimages">
  ...
  <xsl:template match="dc:description" mode="kenyon_bfecimages">
  ...
  <xsl:template match="dc:description.abstract" mode="kenyon_bfecimages">
    <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(replace(replace(., '&lt;[/]*em&gt;', ''), '&lt;[/]*p&gt;', ''))"/>
    </xsl:element>
  </xsl:template>


  <!-- use the dc:identifier to populate edm:isShownAt -->
  <xsl:template match="dc:identifier" mode="kenyon_bfecimages">
    <xsl:choose>
