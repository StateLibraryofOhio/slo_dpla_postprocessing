...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Althea Hurst Scrapbook</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="plochc_p16998coll1"/>      <!-- create edm:isShownAt, edm:preview, and dcterms:identifier -->
      ...
      <xsl:apply-templates select="dc:format"                mode="plochc_p16998coll1"/>      <!-- create dc:format                          -->
      <xsl:apply-templates select="dcterms:spatial"          mode="plochc_p16998coll1"/>      <!-- create dcterms:spatial                    -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="plochc_p16998coll1"/>      <!-- create dcterms:contributor                -->
      <xsl:apply-templates select="dc:description"           mode="plochc_p16998coll1"/>      <!-- create dcterms:description                -->
      <xsl:apply-templates select="dcterms:extent"           mode="plochc_p16998coll1"/>      <!-- create dcterms:extent                     -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="plochc_p16998coll1"/>      <!-- create dcterms:publisher                  -->
      ...
      <xsl:apply-templates select="dc:source"                mode="plochc_p16998coll1"/>      <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:created"          mode="plochc_p16998coll1"/>      <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:license"          mode="plochc_p16998coll1"/>      <!-- unwanted; remove                          -->
  ...
  <xsl:template match="dc:contributor" mode="plochc_p16998coll1"/>

  <xsl:template match="dc:source"  mode="plochc_p16998coll1"/>

  <xsl:template match="dcterms:created" mode="plochc_p16998coll1"/>

  <xsl:template match="dcterms:extent" mode="plochc_p16998coll1"/>

  <xsl:template match="dcterms:license" mode="plochc_p16998coll1"/>

  <xsl:template match="dcterms:spatial" mode="plochc_p16998coll1">
  ...
  <xsl:template match="dc:format" mode="plochc_p16998coll1">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="not(contains(normalize-space(.), 'Bytes')) and normalize-space(.) != 'jpg' and normalize-space(.) != 'pdf' and normalize-space(.) != ''">
  ...
  <xsl:template match="dc:publisher" mode="plochc_p16998coll1">
  ...
  <xsl:template match="dc:identifier" mode="plochc_p16998coll1">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="plochc_p16998coll1">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
