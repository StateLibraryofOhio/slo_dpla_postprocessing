...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Ohio Wesleyan University</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="delawaremem_wesleyan"/>   <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="delawaremem_wesleyan"/>   <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:subject"               mode="delawaremem_wesleyan"/>    <!-- create dcterms:subject                                     -->
      <xsl:apply-templates select="dc:type"                  mode="delawaremem_wesleyan"/>   <!-- create dcterms:type                                        -->
  ...
  <xsl:template match="dc:identifier" mode="delawaremem_wesleyan">
    <xsl:choose>
  ...
  <xsl:template match="dc:subject" mode="delawaremem_wesleyan">
  ...
  <xsl:template match="dc:type" mode="delawaremem_wesleyan">
    <xsl:for-each select=".">
      <xsl:for-each select="tokenize(., '&#xA;')">
        <xsl:choose>
  ...
  <xsl:template match="dc:rights" mode="delawaremem_wesleyan">
    <xsl:for-each select="tokenize(., '&#xA;')">
