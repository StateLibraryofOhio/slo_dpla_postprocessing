...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">New Deal Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="ohmem_p15005coll32"/>      <!-- create edm:rights    and dc:rights                         -->
      <xsl:apply-templates select="dcterms:license"          mode="ohmem_p15005coll32"/>      <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="ohmem_p15005coll32"/>      <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="ohmem_p15005coll32"/>      <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="ohmem_p15005coll32"/>      <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="ohmem_p15005coll32"/>      <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="ohmem_p15005coll32"/>      <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dcterms:temporal"         mode="ohmem_p15005coll32"/>      <!-- create dcterms:temporal                                    -->

      <xsl:apply-templates select="dc:source"                mode="ohmem_p15005coll32"/>      <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:contributor" mode="ohmem_p15005coll32"/>

  <xsl:template match="dc:date" mode="ohmem_p15005coll32"/>

  <xsl:template match="dc:rights" mode="ohmem_p15005coll32"/>

  <xsl:template match="dc:source" mode="ohmem_p15005coll32"/>

  <xsl:template match="dcterms:extent" mode="ohmem_p15005coll32"/>

  <xsl:template match="dcterms:spatial" mode="ohmem_p15005coll32">
  ...
  <xsl:template match="dc:creator" mode="ohmem_p15005coll32">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:license" mode="ohmem_p15005coll32">
    <xsl:element name="edm:rights" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:medium" mode="ohmem_p15005coll32">
    <xsl:element name="dc:format" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:temporal" mode="ohmem_p15005coll32">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
