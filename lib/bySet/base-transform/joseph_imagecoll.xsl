...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:apply-templates select="dc:creator"               mode="joseph_imagecoll"/>        <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="joseph_imagecoll"/>        <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="joseph_imagecoll"/>        <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="joseph_imagecoll"/>        <!-- create dcterms:publisher                                   -->
  ...
  <xsl:template match="dc:publisher" mode="joseph_imagecoll">
  ...
  <xsl:template match="dc:date" mode="joseph_imagecoll">
  ...
  <xsl:template match="dcterms:spatial" mode="joseph_imagecoll">
    <xsl:for-each select="tokenize(normalize-space(.), ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:creator" mode="joseph_imagecoll">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="(normalize-space(.) != '')">
