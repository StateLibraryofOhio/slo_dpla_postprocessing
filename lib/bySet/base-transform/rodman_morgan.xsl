...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Morgan Engineering</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="rodman_morgan"/>           <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="rodman_morgan"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="rodman_morgan"/>           <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="rodman_morgan"/>           <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="rodman_morgan"/>           <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:license"          mode="rodman_morgan"/>           <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dc:coverage" mode="rodman_morgan"/>

  <xsl:template match="dcterms:license" mode="rodman_morgan"/>

  
  <xsl:template match="dc:date" mode="rodman_morgan">
  ...
  <xsl:template match="dc:identifier" mode="rodman_morgan">
    <xsl:choose>
  ...
  <xsl:template match="dc:format" mode="rodman_morgan">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dcterms:spatial" mode="rodman_morgan">
    <xsl:for-each select="tokenize(., ';')">
