...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Columbus in Historic Photographs</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="cml_ohio"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="cml_ohio"/>                <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:type"                  mode="cml_ohio"/>                <!-- create dcterms:type                                        -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative"      mode="odn"/>                     <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="cml_ohio"/>                <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="cml_ohio"/>                <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="odn"/>                     <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cml_ohio"/>                <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_ohio"/>                <!-- create dcterms:temporal                                    -->

      <xsl:apply-templates select="dc:coverage"              mode="cml_ohio"/> 
      <xsl:apply-templates select="dc:source"                mode="cml_ohio"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_ohio"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:issued"           mode="cml_ohio"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isVersionOf"      mode="cml_ohio"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:medium"           mode="cml_ohio"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:modified"         mode="cml_ohio"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_ohio"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:references"       mode="cml_ohio"/>                <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:contributor" mode="cml_ohio"/>
  <xsl:template match="dc:publisher" mode="cml_ohio"/>
  <xsl:template match="dc:source" mode="cml_ohio"/>
  <xsl:template match="dc:rights" mode="cml_ohio"/>
  
  <xsl:template match="dcterms:isPartOf" mode="cml_ohio"/>
  
  <xsl:template match="dcterms:issued" mode="cml_ohio"/>
  <xsl:template match="dcterms:isVersionOf" mode="cml_ohio"/>
  <xsl:template match="dcterms:medium" mode="cml_ohio"/>
  <xsl:template match="dcterms:modified" mode="cml_ohio"/>
  <xsl:template match="dcterms:provenance" mode="cml_ohio"/>
  <xsl:template match="dcterms:references" mode="cml_ohio"/>
  ...
  <xsl:template match="dc:coverage" mode="cml_ohio">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="(normalize-space(.) != '')">
        <xsl:element name="dcterms:spatial" namespace="http://purl.org/dc/terms/">
          <xsl:value-of select="normalize-space(.)"/>
  ...
  <xsl:template match="dc:date" mode="cml_ohio">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="(normalize-space(.) != '')">
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_ohio">
    <xsl:for-each select="tokenize(., ';')">
