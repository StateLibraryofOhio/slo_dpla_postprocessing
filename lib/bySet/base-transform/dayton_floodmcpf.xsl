...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">             <!-- this needs to be changed if the server isn't running CONTENTdm with an oai_qdc metadataPrefix harvest -->
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">1913 Flood Photographs - Montgomery County Photo File</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="dayton_floodmcpf"/>        <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dc:source"                mode="dayton_floodmcpf"/>        <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dc:relation"              mode="dayton_floodmcpf"/>        <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="dayton_floodmcpf"/>

  <xsl:template match="dc:relation" mode="dayton_floodmcpf"/>


  <!-- date values that don't start with "1913" are unwanted, as they're the date-digitized -->
  <xsl:template match="dc:date" mode="dayton_floodmcpf">
    <xsl:choose>
