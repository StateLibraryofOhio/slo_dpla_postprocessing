...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Kent State University Sanborn Fire Insurance maps</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_sanbornmaps"/>        <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="kent_sanbornmaps"/>        <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_sanbornmaps"/>        <!-- create dcterms:description                                 -->
  ...
  <xsl:template match="dc:description" mode="kent_sanbornmaps">
      ...
      <xsl:choose>
        ...
        <xsl:otherwise>
          <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
            <xsl:value-of select="normalize-space(replace(replace(., '&lt;[/]*span&gt;', ''), '&lt;[/]*p&gt;', ''))"/>
          </xsl:element>
        </xsl:otherwise>
      </xsl:choose>
    </xsl:if>
  ...
  <xsl:template match="dc:identifier" mode="kent_sanbornmaps">
    <xsl:element name="edm:isShownAt" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:format" mode="kent_sanbornmaps"/>
