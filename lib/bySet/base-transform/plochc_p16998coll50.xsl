...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">19th Century Photography</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="plochc_p16998coll50"/>     <!-- create edm:isShownAt, edm:preview    -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="plochc_p16998coll50"/>     <!-- create dc:date                            -->
      <xsl:apply-templates select="dc:format"                mode="plochc_p16998coll50"/>     <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:medium"           mode="plochc_p16998coll50"/>     <!-- create dc:format                          -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="plochc_p16998coll50"/>     <!-- create dcterms:extent                     -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="plochc_p16998coll50"/>     <!-- create dcterms:publisher                  -->
      <xsl:apply-templates select="dc:relation"              mode="odn"/>                     <!-- create dc:relation                        -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:apply-templates select="dcterms:rightsHolder"     mode="plochc_p16998coll50"/>     <!-- create dcterms:rightsHolder               -->
      <xsl:apply-templates select="dcterms:temporal"         mode="odn"/>                     <!-- create dcterms:temporal                   -->

      <xsl:apply-templates select="dc:source"                mode="plochc_p16998coll50"/>     <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:created"          mode="plochc_p16998coll50"/>     <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="plochc_p16998coll50"/>     <!-- unwanted; remove                          -->
      <xsl:apply-templates select="dcterms:license"          mode="plochc_p16998coll50"/>     <!-- unwanted; remove                          -->

  </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:publisher" mode="plochc_p16998coll50"/>

  <xsl:template match="dc:source" mode="plochc_p16998coll50"/>

  <xsl:template match="dcterms:created" mode="plochc_p16998coll50"/>

  <xsl:template match="dcterms:extent" mode="plochc_p16998coll50"/>

  <xsl:template match="dcterms:isPartOf" mode="plochc_p16998coll50"/>

  <xsl:template match="dcterms:license" mode="plochc_p16998coll50"/>

  <xsl:template match="dcterms:rightsHolder" mode="plochc_p16998coll50"/>

  <xsl:template match="dc:format" mode="plochc_p16998coll50"/>
  ...
  <xsl:template match="dc:identifier" mode="plochc_p16998coll50">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:medium" mode="plochc_p16998coll50">
    <xsl:element name="dc:format" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:date" mode="plochc_p16998coll50">
