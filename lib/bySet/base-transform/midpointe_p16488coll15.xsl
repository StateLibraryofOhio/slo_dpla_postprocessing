...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <!-- Change the "midpoint01" and "MidPointe Library System" values -->
  ...
  <xsl:template match="text()|@*"/>
    <xsl:template match="//oai_qdc:qualifieddc">
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Armco Ambulance Corps Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"    mode="midpointe_p16488coll15"/>  <!-- create edm:isShownAt and dcterms:identifier -->
      ...
      <xsl:apply-templates select="dc:language"         mode="midpointe_p16488coll15"/>  <!-- create dcterms:language                 -->
      <xsl:apply-templates select="dc:creator"          mode="midpointe_p16488coll15"/>  <!-- create dcterms:creator                  -->
      <xsl:copy-of         select="dc:date"             copy-namespaces="no"/>           <!-- create dc:date                          -->
      <xsl:apply-templates select="dcterms:created"     mode="midpointe_p16488coll15"/>  <!-- create dc:date                          -->
      <xsl:apply-templates select="dc:format"           mode="midpointe_p16488coll15"/>  <!-- create dc:format                        -->
      <xsl:copy-of         select="dcterms:spatial"     copy-namespaces="no"/>           <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:coverage"         mode="midpointe_p16488coll15"/>  <!-- create dcterms:spatial                  -->
      ...
      <xsl:apply-templates select="dc:source"           mode="midpointe_p16488coll15"/>  <!-- create dc:relation          -->
  ...
  <xsl:template match="dc:creator" mode="midpointe_p16488coll15">
  ...
  <xsl:template match="dc:language" mode="midpointe_p16488coll15">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll15">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:source" mode="midpointe_p16488coll15">
    <xsl:element name="dc:relation" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:coverage" mode="midpointe_p16488coll15">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:created" mode="midpointe_p16488coll15">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="midpointe_p16488coll15">
    <xsl:choose>
