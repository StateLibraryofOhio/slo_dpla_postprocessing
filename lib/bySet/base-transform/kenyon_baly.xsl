...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Baly Slide Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kenyon_baly"/>             <!-- create edm:isShownAt, edm:preview, and dcterms:identifier -->
      <xsl:apply-templates select="dc:rights"                mode="odn"/>                     <!-- create edm:rights    and dc:rights   -->
      <xsl:apply-templates select="dc:title"                 mode="kenyon_baly"/>             <!-- create dcterms:title -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="kenyon_baly"/>             <!-- create dc:date                            -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial"      mode="kenyon_baly"/>             <!-- create dcterms:spatial                    -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kenyon_baly"/>             <!-- create edm:preview                        -->
      <xsl:apply-templates select="dc:description.abstract"  mode="kenyon_baly"/>             <!-- create dcterms:description                -->
  ...
  <xsl:template match="dc:title" mode="kenyon_baly">
    <xsl:element name="dcterms:title" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="replace(., '^B\d\d.\d\d\d? ', '')"/>
    </xsl:element>
  </xsl:template>


  <!-- use the dc:identifier to populate edm:isShownAt -->
  <xsl:template match="dc:identifier" mode="kenyon_baly">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="kenyon_baly">
  ...
  <xsl:template match="dc:date.created" mode="kenyon_baly">
  ...
  <xsl:template match="dc:coverage.spatial" mode="kenyon_baly">
  ...
  <!-- removing HTML from metadata...P and EM tags -->
  <xsl:template match="dc:description.abstract" mode="kenyon_baly">
    <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(replace(replace(replace(., '&lt;[/]*sup&gt;', ''), '&lt;[/]*em&gt;', ''), '&lt;[/]*p&gt;', ''))"/>
    </xsl:element>
  </xsl:template>
