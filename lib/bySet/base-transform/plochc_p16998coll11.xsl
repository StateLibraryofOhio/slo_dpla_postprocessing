...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:include href="odn_templates.xsl"/>
    ...
    <xsl:template match="//oai_qdc:qualifieddc">
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Ohio River Floods</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"       mode="plochc_p16998coll11"/>            <!-- create edm:isShownAt, edm:preview -->
      ...
      <xsl:apply-templates select="dc:date"             mode="plochc_p16998coll11"/>            <!-- create dc:date                          -->
      <xsl:apply-templates select="dc:format"           mode="plochc_p16998coll11"/>            <!-- create dc:format                        -->
      <xsl:apply-templates select="dcterms:spatial"     mode="plochc_p16998coll11"/>            <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:subject"          mode="odn"/>                            <!-- create dcterms:subject                  -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative" mode="plochc_p16998coll11"/>            <!-- create dcterms:alternative  -->
      <xsl:apply-templates select="dc:contributor"      mode="plochc_p16998coll11"/>            <!-- create dcterms:contributor  -->
      <xsl:apply-templates select="dc:description"      mode="odn"/>                            <!-- create dcterms:description  -->
      <xsl:apply-templates select="dcterms:extent"      mode="plochc_p16998coll11"/>            <!-- create dcterms:extent       -->
                                                                                                <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"        mode="plochc_p16998coll11"/>            <!-- create dcterms:publisher    -->
      <xsl:apply-templates select="dc:relation"         mode="odn"/>                            <!-- create dc:relation          -->
                                                                                                <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:apply-templates select="dcterms:rightsHolder"     mode="plochc_p16998coll11"/>       <!-- create dcterms:rightsHolder -->
      <xsl:apply-templates select="dcterms:temporal"         mode="odn"/>                       <!-- create dcterms:temporal     -->

      <xsl:apply-templates select="dcterms:medium"      mode="plochc_p16998coll11"/>            <!-- unwanted; remove            -->

      </oai_qdc:qualifieddc>
    </xsl:template>
  

  <xsl:template match="dcterms:medium"  mode="plochc_p16998coll11"/>

  <xsl:template match="dcterms:rightsHolder" mode="plochc_p16998coll11"/>

  <xsl:template match="dc:contributor" mode="plochc_p16998coll11"/>

  <xsl:template match="dcterms:alternative" mode="plochc_p16998coll11"/>

  <xsl:template match="dcterms:extent" mode="plochc_p16998coll11"/>

  <xsl:template match="dc:publisher" mode="plochc_p16998coll11"/>

  <xsl:template match="dc:format" mode="plochc_p16998coll11"/>

  <xsl:template match="dcterms:spatial" mode="plochc_p16998coll11">
  ...
  <xsl:template match="dc:identifier" mode="plochc_p16998coll11">
    <xsl:choose>
  ...
  <xsl:template match="dc:date" mode="plochc_p16998coll11">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
