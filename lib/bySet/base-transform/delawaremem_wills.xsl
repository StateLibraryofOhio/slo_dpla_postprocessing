...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Delaware County Ohio Will Records</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="delawaremem_wills"/>   <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="delawaremem_wills"/>   <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:type"                  mode="delawaremem_wills"/>   <!-- create dcterms:type                                        -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative"      mode="odn"/>                     <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="delawaremem_wills"/>       <!-- create dcterms:contributor                                 -->
  ...
  <xsl:template match="dc:identifier" mode="delawaremem_wills">
    <xsl:choose>
  ...
  <xsl:template match="dc:type" mode="delawaremem_wills">
    <xsl:for-each select=".">
  ...
  <xsl:template match="dc:contributor" mode="delawaremem_wills">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:rights" mode="delawaremem_wills">
    <xsl:for-each select="tokenize(., '&#xA;')">
