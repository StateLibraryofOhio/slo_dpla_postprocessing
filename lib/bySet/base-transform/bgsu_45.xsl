...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Northwest Ohio Photographs Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_45"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="bgsu_45"/>                 <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="bgsu_45"/>                 <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="bgsu_45"/>                 <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_45"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:provenance"       mode="bgsu_45"/>                 <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:created" mode="bgsu_45"/>

  <xsl:template match="dcterms:provenance" mode="bgsu_45"/>

  <xsl:template match="dc:format" mode="bgsu_45">
  ...
  <xsl:template match="dc:subject" mode="bgsu_45">
    <xsl:element name="dcterms:subject" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(replace(., '[ ]*--[ ]*', '--'))"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="bgsu_45">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:isPartOf" mode="bgsu_45">
