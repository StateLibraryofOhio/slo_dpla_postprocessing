...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">New University Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_50"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="odn"/>                     <!-- create edm:rights    and dc:rights                         -->
      <xsl:apply-templates select="dc:title"                 mode="bgsu_50"/>                 <!-- create dcterms:title                                       -->
      ...
      <xsl:apply-templates select="dc:format"                mode="bgsu_50"/>                 <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="bgsu_50"/>                 <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="bgsu_50"/>                 <!-- create dc:relation                                         -->
  ...
  <xsl:template match="dc:format" mode="bgsu_50">
    <xsl:if test="normalize-space(.) != 'application/pdf' and normalize-space(.) != 'image/jpeg'">
  ...
  <xsl:template match="dc:title" mode="bgsu_50">
    <xsl:element namespace="http://purl.org/dc/terms/" name="dcterms:title">
      <xsl:value-of select="replace(normalize-space(.), '&lt;[/]*em&gt;', '')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:subject" mode="bgsu_50">
    <xsl:element name="dcterms:subject" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(replace(., '[ ]*--[ ]*', '--'))"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="bgsu_50">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:isPartOf" mode="bgsu_50">
