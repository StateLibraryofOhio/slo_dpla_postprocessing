...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:apply-templates select="dc:date"                  mode="midpointe_p16488coll21"/>  <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="midpointe_p16488coll21"/>  <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="midpointe_p16488coll21"/>  <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dcterms:issued"           mode="midpointe_p16488coll21"/>  <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:coverage"              mode="midpointe_p16488coll21"/>  <!-- create dcterms:spatial                                     -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="midpointe_p16488coll21">
    <xsl:choose>
  ...
  <xsl:template match="dc:date" mode="midpointe_p16488coll21">
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll21">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:contributor" mode="midpointe_p16488coll21">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:issued" mode="midpointe_p16488coll21">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
        <xsl:value-of select="."/>
      </xsl:element>
  ...
  <xsl:template match="dc:coverage" mode="midpointe_p16488coll21">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
