...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Ohio Guide Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dcterms:license"          mode="ohmem_p267401coll34"/>     <!-- create edm:rights    and dc:rights                         -->
      <xsl:apply-templates select="dc:rights"                mode="ohmem_p267401coll34"/>     <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:language"              mode="ohmem_p267401coll34"/>     <!-- create dcterms:language                                    -->
      <xsl:apply-templates select="dc:creator"               mode="ohmem_p267401coll34"/>     <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="ohmem_p267401coll34"/>     <!-- create dc:date                                             -->
      <xsl:apply-templates select="dcterms:medium"           mode="ohmem_p267401coll34"/>     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="ohmem_p267401coll34"/>     <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="ohmem_p267401coll34"/>     <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="ohmem_p267401coll34"/>     <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="ohmem_p267401coll34"/>     <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="ohmem_p267401coll34"/>     <!-- create dc:date                                             -->

      <xsl:apply-templates select="dc:source"                mode="ohmem_p267401coll34"/>     <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dc:creator" mode="ohmem_p267401coll34">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:license" mode="ohmem_p267401coll34">
    <xsl:element name="edm:rights" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:medium" mode="ohmem_p267401coll34">
  ...
  <xsl:template match="dcterms:temporal" mode="ohmem_p267401coll34">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
