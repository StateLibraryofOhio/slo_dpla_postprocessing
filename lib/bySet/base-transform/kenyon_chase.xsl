...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Philander Chase Letters</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kenyon_chase"/>            <!-- create edm:isShownAt, edm:preview, and dcterms:identifier -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="kenyon_chase"/>            <!-- create dc:format                          -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial"      mode="kenyon_chase"/>            <!-- create dcterms:subject                    -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kenyon_chase"/>            <!-- create edm:preview                        -->
      <xsl:apply-templates select="dc:description.abstract"  mode="kenyon_chase"/>            <!-- create edm:preview                        -->
  ...
  <xsl:template match="dc:date.created" mode="kenyon_chase">
  ...
  <xsl:template match="dc:identifier" mode="kenyon_chase">
    <xsl:if test="starts-with(., 'http') and not(contains(., 'viewcontent'))">
