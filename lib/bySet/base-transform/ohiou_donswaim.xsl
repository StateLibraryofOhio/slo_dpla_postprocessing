...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Don Swaim Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="ohiou_donswaim"/>          <!-- create dcterms:creator                                     -->
      <xsl:copy-of         select="dc:date"                  copy-namespaces="no"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="ohiou_donswaim"/>          <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="ohiou_donswaim"/>          <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dc:relation"              mode="ohiou_donswaim"/>          <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dc:source"                mode="ohiou_donswaim"/>
      <xsl:apply-templates select="dcterms:bibliographicCitation" mode="ohiou_donswaim"/>
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="ohiou_donswaim"/>
      <xsl:apply-templates select="dcterms:references"       mode="ohiou_donswaim"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="ohiou_donswaim"/>
  <xsl:template match="dcterms:bibliographicCitation" mode="ohiou_donswaim"/>
  <xsl:template match="dcterms:isReferencedBy" mode="ohiou_donswaim"/>
  <xsl:template match="dcterms:references" mode="ohiou_donswaim"/>


  <xsl:template match="dc:format" mode="ohiou_donswaim">
  ...
  <xsl:template match="dc:creator" mode="ohiou_donswaim">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:subject" mode="ohiou_donswaim">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:relation" mode="ohiou_donswaim">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
