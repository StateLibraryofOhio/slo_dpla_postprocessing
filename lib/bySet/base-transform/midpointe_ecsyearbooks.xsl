...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Edgewood City Schools and Trenton Yearbooks</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="midpointe_ecsyearbooks"/>  <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="midpointe_ecsyearbooks"/>  <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="odn"/>                     <!-- eliminate by default; use dcterms:isReferencedBy for Wikimedia data  -->      <xsl:apply-templates select="dc:publisher"             mode="odn"/>                     <!-- create dcterms:publisher                                   -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="midpointe_ecsyearbooks"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:coverage" mode="midpointe_ecsyearbooks">
  ...
  <xsl:template match="dc:format" mode="midpointe_ecsyearbooks">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="midpointe_ecsyearbooks">
    <xsl:choose>
