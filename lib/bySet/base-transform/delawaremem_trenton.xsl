...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Trenton Township</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="delawaremem_trenton"/>        <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:subject"               mode="delawaremem_trenton"/>     <!-- create dcterms:subject                                     -->
      <xsl:apply-templates select="dc:type"                  mode="delawaremem_trenton"/>        <!-- create dcterms:type                                        -->
      ...
      <xsl:apply-templates select="dc:source"                mode="delawaremem_trenton"/>     <!-- frequently unused; remove by default                       -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="delawaremem_trenton">
     <xsl:element name="dc:publisher" namespace="http://purl.org/dc/elements/1.1/">
       <xsl:value-of select="normalize-space(.)"/>
     </xsl:element>
  </xsl:template>

  <xsl:template match="dc:subject" mode="delawaremem_trenton">
  ...
  <xsl:template match="dc:identifier" mode="delawaremem_trenton">
    <xsl:choose>
  ...
  <xsl:template match="dc:type" mode="delawaremem_trenton">
    <xsl:for-each select=".">
      <xsl:for-each select="tokenize(., '&#xA;')">
        <xsl:choose>
