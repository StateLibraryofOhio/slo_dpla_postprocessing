...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Grace McClurg Carson papers (MS-0243)</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_171"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="bgsu_171"/>                <!-- create dcterms:creator                                     -->
      ...
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="bgsu_171"/>                <!-- create IIIF metadata                                       -->
      <xsl:apply-templates select="dc:publisher"             mode="bgsu_171"/>                <!-- create dcterms:publisher                                   -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_171"/>                <!-- unused; remove                                             -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:created" mode="bgsu_171"/>
  <xsl:template match="dcterms:isReferencedBy" mode="bgsu_171"/>
  

  <xsl:template match="dc:creator" mode="bgsu_171">
  ...
  <xsl:template match="dc:publisher" mode="bgsu_171">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="bgsu_171">
    <xsl:choose>
