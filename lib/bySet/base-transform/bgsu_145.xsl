...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Letters and diaries from Northwest Ohio</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_145"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="bgsu_145"/>                <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:relation"              mode="bgsu_145"/>                <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_145"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_145"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="bgsu_145"/>                <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dc:source" mode="bgsu_145"/>

  <xsl:template match="dcterms:created" mode="bgsu_145"/>
  
  <xsl:template match="dc:relation" mode="bgsu_145"/>

  <xsl:template match="dcterms:isReferencedBy" mode="bgsu_145"/>

  <xsl:template match="dc:identifier" mode="bgsu_145">
    <xsl:choose>
  ...
  <xsl:template match="dc:description" mode="bgsu_145">
