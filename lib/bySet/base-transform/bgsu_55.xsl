...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">James Whitmer letter</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_55"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:language"              mode="bgsu_55"/>                 <!-- create dcterms:language                                    -->
      ...
      <xsl:apply-templates select="dc:format"                mode="bgsu_55"/>                 <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_55"/>                 <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="bgsu_55"/>                 <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="bgsu_55"/>

  <xsl:template match="dcterms:isReferencedBy" mode="bgsu_55"/>

  <xsl:template match="dc:identifier" mode="bgsu_55">
    <xsl:choose>
  ...
  <xsl:template match="dc:format" mode="bgsu_55">
    <xsl:if test="not(matches(normalize-space(.), 'application/pdf'))">
  ...
  <xsl:template match="dc:language" mode="bgsu_55">
