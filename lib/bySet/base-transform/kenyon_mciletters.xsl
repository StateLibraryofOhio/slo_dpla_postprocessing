...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Charles Petit McIlvaine Letters</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kenyon_mciletters"/>       <!-- create edm:isShownAt, edm:preview, and dcterms:identifier -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="kenyon_mciletters"/>       <!-- create dc:format                          -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kenyon_mciletters"/>       <!-- create edm:preview                        -->
      <xsl:apply-templates select="dc:description.abstract"  mode="kenyon_mciletters"/>       <!-- create dcterms:description                -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial"      mode="kenyon_mciletters"/>       <!-- unwanted; remove                          -->
    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:coverage.spatial" mode="kenyon_mciletters"/>

  <xsl:template match="dc:description" mode="kenyon_mciletters">
  ...
  <xsl:template match="dc:description.abstract" mode="kenyon_mciletters">
    <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(replace(., '&lt;[/]*p&gt;', ''))"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:date.created" mode="kenyon_mciletters">
  ...
  <xsl:template match="dc:identifier" mode="kenyon_mciletters">
    <xsl:choose>
