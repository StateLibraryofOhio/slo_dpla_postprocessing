...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">             <!-- this may need to be changed if the server isn't running CONTENTdm -->
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">City of Middletown Slides Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="midpointe_p16488coll10"/>  <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="midpointe_p16488coll10"/>  <!-- create dcterms:creator                                     -->
      <xsl:copy-of         select="dc:date"                  copy-namespaces="no"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="midpointe_p16488coll10"/>  <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="midpointe_p16488coll10"/>  <!-- create dcterms:contributor                                 -->
      ...
      <xsl:apply-templates select="dc:coverage"              mode="midpointe_p16488coll10"/>  <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dcterms:license"          mode="midpointe_p16488coll10"/>  <!-- create dcterms:license                                     -->

      <xsl:apply-templates select="dc:source"                mode="midpointe_p16488coll10"/>  <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:source" mode="midpointe_p16488coll10"/>


  <xsl:template match="dc:identifier" mode="midpointe_p16488coll10">
    <xsl:choose>
  ...
  <xsl:template match="dc:contributor" mode="midpointe_p16488coll10">
  ...
  <xsl:template match="dc:creator" mode="midpointe_p16488coll10">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll10">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != '' and normalize-space(.) != 'Image/JPEG'">
  ...
  <xsl:template match="dc:coverage" mode="midpointe_p16488coll10">
    <xsl:element name="dcterms:spatial" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:license" mode="midpointe_p16488coll10">
