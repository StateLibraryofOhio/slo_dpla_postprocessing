...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Buildings and Grounds Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="wooster_buildingsgrounds"/>  <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="wooster_buildingsgrounds"/>  <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="wooster_buildingsgrounds"/>  <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial.lat"  mode="wooster_buildingsgrounds"/>  <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:description"           mode="wooster_buildingsgrounds"/>  <!-- create edm:preview                                         -->
      <xsl:apply-templates select="dc:description.abstract"  mode="wooster_buildingsgrounds"/>  <!-- create dcterms:description                                 -->
  ...
  <xsl:template match="dc:date.created" mode="wooster_buildingsgrounds">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="substring-before(normalize-space(.),'T')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:description" mode="wooster_buildingsgrounds">
...
-->
  ...
  <xsl:template match="dc:rights" mode="wooster_buildingsgrounds"><xsl:element name="edm:rights" namespace="http://www.europeana.eu/schemas/edm/">http<xsl:value-of select="normalize-space(substring-after(substring-before(., '/1.0/'), 'http'))" />/1.0/</xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="wooster_buildingsgrounds">
    <xsl:choose>
