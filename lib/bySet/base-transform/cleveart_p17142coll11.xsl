...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Cleveland Museum of Art Archives</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Fine Arts Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="cleveart_p17142coll11"/>   <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="cleveart_p17142coll11"/>   <!-- create dcterms:creator                                     -->
      <xsl:copy-of         select="dc:date"                  copy-namespaces="no"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="cleveart_p17142coll11"/>  <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="cleveart_p17142coll11"/>   <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dc:contributor"           mode="cleveart_p17142coll11"/>   <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="cleveart_p17142coll11"/>   <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dcterms:bibliographicCitation"  mode="cleveart_p17142coll11"/>   <!-- unwanted; remove  -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:format" mode="cleveart_p17142coll11"/>
  <xsl:template match="dcterms:bibliographicCitation" mode="cleveart_p17142coll11"/>

  <xsl:template match="dc:subject" mode="cleveart_p17142coll11">
  ...
  <xsl:template match="dc:description" mode="cleveart_p17142coll11">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:creator" mode="cleveart_p17142coll11">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:contributor" mode="cleveart_p17142coll11">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:rights" mode="cleveart_p17142coll11">
    <xsl:for-each select="tokenize(., ',')">
