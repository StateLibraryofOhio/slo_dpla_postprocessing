...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Ella P. Stewart scrapbooks</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_143"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="odn"/>                     <!-- create edm:rights    and dc:rights                         -->
      <xsl:apply-templates select="dc:title"                 mode="bgsu_143"/>                <!-- create dcterms:title                                       -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_143"/>                <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_143"/>                <!-- unwanted; remove                                           -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="bgsu_143"/>
  
  <xsl:template match="dcterms:created" mode="bgsu_143"/>

  <xsl:template match="dc:title" mode="bgsu_143">
    <xsl:element name="dcterms:title" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="replace(normalize-space(.), '&lt;[/]*em&gt;', '')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="bgsu_143">
    <xsl:choose>
