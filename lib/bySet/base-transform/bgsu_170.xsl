...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Dora E. Giffen papers (MS-0309)</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_170"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:language"              mode="bgsu_170"/>                <!-- create dcterms:language                                    -->
      <xsl:apply-templates select="dc:creator"               mode="bgsu_170"/>                <!-- create dcterms:creator                                     -->
      ...
      <xsl:apply-templates select="dcterms:spatial"          mode="bgsu_170"/>                <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="bgsu_170"/>                <!-- eliminate; it conflicts with ODN IIIF metadata                                       -->
      ...
      <xsl:apply-templates select="dcterms:created"          mode="bgsu_170"/>
  ...
  <xsl:template match="dcterms:spatial" mode="bgsu_170">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:creator" mode="bgsu_170">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:identifier" mode="bgsu_170">
    <xsl:choose>
