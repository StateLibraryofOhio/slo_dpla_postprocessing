...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Columbus State Community College</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Columbus State Digital Archive</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="cscc_p16007coll130"/>      <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="cscc_p16007coll130"/>      <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="cscc_p16007coll130"/>      <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="cscc_p16007coll130"/>
      <xsl:apply-templates select="dcterms:license"          mode="cscc_p16007coll130"/>      <!-- create edm:rights -->
  ...
  <xsl:template match="dc:creator" mode="cscc_p16007coll130">
  ...
  <xsl:template match="dcterms:spatial" mode="cscc_p16007coll130">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
