...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Kaleidoscope</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_kaleidoscope"/>       <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_kaleidoscope"/>       <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_kaleidoscope"/>       <!-- sending to dc:relation                                     -->

    </oai_qdc:qualifieddc>
  </xsl:template>


  <xsl:template match="dc:source" mode="kent_kaleidoscope">
    <xsl:element name="dc:relation" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:description" mode="kent_kaleidoscope">
  ...
  <xsl:template match="dc:identifier" mode="kent_kaleidoscope">
