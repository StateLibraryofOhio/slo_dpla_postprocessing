...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">My Happy Institution</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">The History of Dustmites</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dcterms:extent"           mode="test_p16998coll9"/>        <!-- create dcterms:extent                                      -->
      ...
      <xsl:apply-templates select="dc:relation"              mode="test_p16998coll9"/>        <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dcterms:medium"           mode="test_p16998coll9"/>
      <xsl:apply-templates select="dcterms:license"          mode="test_p16998coll9"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <!-- we don't want these...eliminate them -->
  <xsl:template match="dcterms:medium"          mode="test_p16998coll9"/>
  <xsl:template match="dcterms:extent"          mode="test_p16998coll9"/>
  <xsl:template match="dc:relation"             mode="test_p16998coll9"/>
  <xsl:template match="dcterms:license"         mode="test_p16998coll9"/>
  <xsl:template match="dcterms:tableOfContents" mode="test_p16998coll9"/>
  <xsl:template match="dcterms:references"      mode="test_p16998coll9"/>
  ...
  <xsl:template match="dc:identifier"           mode="test_p16998coll9">
    <xsl:choose>
