...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Presidents Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="wooster_presidents"/>      <!-- create edm:isShownAt                                       -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="wooster_presidents"/>      <!-- create dc:date                                             -->
      ...
      <xsl:apply-templates select="dc:coverage.spatial"      mode="wooster_presidents"/>      <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="odn"/>                     <!-- create dcterms:subject                                     -->
      <xsl:apply-templates select="dc:type"                  mode="wooster_presidents"/>      <!-- create dcterms:type                                        -->
      ...
      <xsl:apply-templates select="dc:description"           mode="wooster_presidents"/>      <!-- create edm:preview                                         -->
      <xsl:apply-templates select="dc:description.abstract"  mode="wooster_presidents"/>      <!-- create dc:description                                      -->
  ...
  <xsl:template match="dc:type" mode="wooster_presidents"/>

  <xsl:template match="dc:date.created" mode="wooster_presidents">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="substring-before(normalize-space(.),'T')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:coverage.spatial" mode="wooster_presidents">
    <xsl:element namespace="http://purl.org/dc/terms/" name="dcterms:spatial">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:description.abstract" mode="wooster_presidents">
    <xsl:element name="dcterms:description" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="replace(replace(replace(replace(replace(., '&lt;[/]*strong&gt;', ''), '&lt;[/]*sup&gt;', ''), '&lt;[/]*ol&gt;', ''), '&lt;[/]*li&gt;', ''), '&lt;[/]*p&gt;', '')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="wooster_presidents">
    <xsl:element name="edm:isShownAt" namespace="http://www.europeana.eu/schemas/edm/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:description" mode="wooster_presidents">
