...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Islamic Center of Greater Toledo oral histories</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_154"/>                <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:source"                mode="bgsu_154"/>                <!-- unwanted; remove                              -->

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dc:source" mode="bgsu_154"/>
  ...
  <xsl:template match="dc:identifier" mode="bgsu_154">
    <xsl:if test="starts-with(., 'http://') or starts-with(., 'https://')">
      <xsl:choose>
        <xsl:when test="contains(., 'files/thumbnails')">
