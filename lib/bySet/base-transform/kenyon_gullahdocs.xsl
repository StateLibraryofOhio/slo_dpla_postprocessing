...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Kenyon Gullah Digital Archive Project Documents</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kenyon_gullahdocs"/>       <!-- create edm:isShownAt                      -->
      ...
      <xsl:apply-templates select="dc:date.created"          mode="kenyon_gullahdocs"/>       <!-- create dc:date                            -->
  ...
  <xsl:template match="dc:date.created" mode="kenyon_gullahdocs">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="substring-before(., 'T')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="kenyon_gullahdocs">
    <xsl:choose>
