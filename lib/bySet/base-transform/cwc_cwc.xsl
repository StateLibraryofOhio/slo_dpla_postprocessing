...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Clintonville Woman's Club</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Clintonville Woman's Club</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="cwc_cwc"/>                 <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="cwc_cwc"/>                 <!-- create dcterms:creator                                     -->
      ...
      <xsl:apply-templates select="dcterms:spatial"          mode="cwc_cwc"/>                 <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dcterms:temporal"         mode="cwc_cwc"/>                 <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->

      <xsl:apply-templates select="dcterms:accessRights"     mode="cwc_cwc"/>
      <xsl:apply-templates select="dcterms:issued"           mode="cwc_cwc"/>
      <xsl:apply-templates select="dc:coverage"              mode="cwc_cwc"/>
      <xsl:apply-templates select="dcterms:medium"           mode="cwc_cwc"/>
      <xsl:apply-templates select="dcterms:provenance"       mode="cwc_cwc"/>
      <xsl:apply-templates select="dcterms:mediator"         mode="cwc_cwc"/> 
      <xsl:apply-templates select="dcterms:references"       mode="cwc_cwc"/>
      <xsl:apply-templates select="dcterms:isVersionOf"      mode="cwc_cwc"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:issued" mode="cwc_cwc"/>
  <xsl:template match="dcterms:temporal" mode="cwc_cwc"/>
  <xsl:template match="dcterms:medium" mode="cwc_cwc"/>
  <xsl:template match="dcterms:provenance" mode="cwc_cwc"/>
  <xsl:template match="dc:rights" mode="cwc_cwc"/>
  <xsl:template match="dcterms:mediator" mode="cwc_cwc"/>
  <xsl:template match="dcterms:references" mode="cwc_cwc"/>
  <xsl:template match="dcterms:isVersionOf" mode="cwc_cwc"/>
  <xsl:template match="dcterms:spatial" mode="cwc_cwc"/>

  <xsl:template match="dc:coverage" mode="cwc_cwc">
  ...
  <xsl:template match="dc:creator" mode="cwc_cwc">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:accessRights" mode="cwc_cwc">
    <xsl:for-each select="tokenize(., ';')">
