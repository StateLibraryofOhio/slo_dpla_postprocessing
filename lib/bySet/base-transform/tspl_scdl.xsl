...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="edm:dataProvider" namespace="http://www.europeana.eu/schemas/edm/">Tiffin-Seneca Public Library</xsl:element>     <!-- create edm:dataProvider -->
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Seneca County Digital Library</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="tspl_scdl"/>               <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      <xsl:apply-templates select="dc:rights"                mode="tspl_scdl"/>               <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dc:creator"               mode="tspl_scdl"/>               <!-- create dcterms:creator                                     -->
      <xsl:apply-templates select="dc:date"                  mode="tspl_scdl"/>               <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="odn"/>                     <!-- create dc:format                                           -->
      <xsl:apply-templates select="dcterms:spatial"          mode="tspl_scdl"/>               <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="tspl_scdl"/>               <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dcterms:temporal"         mode="tspl_scdl"/>               <!-- create dcterms:temporal                                    -->

      <xsl:apply-templates select="dcterms:license"          mode="tspl_scdl"/>               <!-- create edm:rights                                          -->
      <xsl:apply-templates select="dc:source"                mode="tspl_scdl"/>               <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="tspl_scdl"/>               <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:medium"           mode="tspl_scdl"/>               <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dc:identifier" mode="tspl_scdl">
    <xsl:choose>
  ...
  <xsl:template match="dc:rights" mode="tspl_scdl" />
  <xsl:template match="dcterms:isPartOf" mode="tspl_scdl" />
  <xsl:template match="dcterms:medium" mode="tspl_scdl" />
  <xsl:template match="dc:source" mode="tspl_scdl" />

  <xsl:template match="dcterms:temporal" mode="tspl_scdl">
  ...
  <xsl:template match="dc:subject" mode="tspl_scdl">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:relation" mode="tspl_scdl">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:spatial" mode="tspl_scdl">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:contributor" mode="tspl_scdl">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:creator" mode="tspl_scdl">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:license" mode="tspl_scdl">
    <xsl:for-each select="tokenize(., ';')">
  ...
  <xsl:template match="dc:date" mode="tspl_scdl">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
        <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
          <xsl:value-of select="normalize-space(.)"/>
