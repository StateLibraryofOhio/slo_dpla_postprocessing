...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Cornelius Ryan Collection of World War II Papers</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:language"              mode="ohiou_p15808coll15"/>      <!-- create dcterms:language                                    -->
      <xsl:apply-templates select="dc:creator"               mode="ohiou_p15808coll15"/>      <!-- create dcterms:creator                                     -->
      <xsl:copy-of         select="dc:date"                  copy-namespaces="no"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="ohiou_p15808coll15"/>      <!-- create dc:format                                           -->
      ...
      <xsl:apply-templates select="dc:publisher"             mode="ohiou_p15808coll15"/>      <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="ohiou_p15808coll15"/>      <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="ohiou_p15808coll15"/>      <!-- create dc:relation                                         -->
      ...
      <xsl:apply-templates select="dc:source"                mode="ohiou_p15808coll15"/>
      <xsl:apply-templates select="dcterms:isPartOf"         mode="ohiou_p15808coll15"/>
      <xsl:apply-templates select="dcterms:isReferencedBy"   mode="ohiou_p15808coll15"/>
  ...
  <xsl:template match="dc:source" mode="ohiou_p15808coll15"/>
  <xsl:template match="dcterms:isPartOf" mode="ohiou_p15808coll15"/>
  <xsl:template match="dcterms:isReferencedBy" mode="ohiou_p15808coll15"/>




  <xsl:template match="dc:language" mode="ohiou_p15808coll15">
  ...
  <xsl:template match="dc:creator" mode="ohiou_p15808coll15">
    <xsl:for-each  select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:format" mode="ohiou_p15808coll15">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:publisher" mode="ohiou_p15808coll15">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
