...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="text()|@*"/>
    <xsl:template match="//oai_qdc:qualifieddc">
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">The Virginia Shewalter Letters Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:language"         mode="midpointe_mpd01"/>         <!-- create dcterms:language                 -->
      <xsl:apply-templates select="dc:creator"          mode="odn"/>                     <!-- create dcterms:creator                  -->
      <xsl:apply-templates select="dc:date"             mode="midpointe_mpd01"/>         <!-- create dc:date                          -->
      <xsl:apply-templates select="dc:format"           mode="midpointe_mpd01"/>         <!-- create dc:format                        -->
      <xsl:copy-of         select="dcterms:spatial"     copy-namespaces="no"/>           <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:coverage"         mode="midpointe_mpd01"/>         <!-- create dcterms:spatial                  -->
      <xsl:apply-templates select="dc:subject"          mode="midpointe_mpd01"/>         <!-- create dcterms:subject                  -->

      <!-- OPTIONAL ODN-MAP fields -->
      <xsl:apply-templates select="dcterms:alternative" mode="odn"/>                     <!-- create dcterms:alternative            -->
      <xsl:apply-templates select="dc:contributor"      mode="midpointe_mpd01"/>         <!-- create dcterms:contributor            -->
      <xsl:apply-templates select="dc:description"      mode="odn"/>                     <!-- create dcterms:description            -->
      <xsl:apply-templates select="dcterms:extent"      mode="midpointe_mpd01"/>         <!-- create dcterms:extent                 -->
      ...
      <xsl:apply-templates select="dc:source"           mode="midpointe_mpd01"/>         <!-- create dc:relation                    -->
  ...
  <xsl:template match="dc:language" mode="midpointe_mpd01">
  ...
  <xsl:template match="dcterms:extent" mode="midpointe_mpd01">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:subject" mode="midpointe_mpd01">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:coverage" mode="midpointe_mpd01">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:source" mode="midpointe_mpd01">
    <xsl:element name="dc:relation" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="normalize-space(.)"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:format" mode="midpointe_mpd01">
  ...
  <xsl:template match="dc:contributor" mode="midpointe_mpd01">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:date" mode="midpointe_mpd01">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
