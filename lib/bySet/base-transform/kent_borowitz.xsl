...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_dc:dc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">The Borowitz True Crime Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="kent_borowitz"/>           <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="kent_borowitz"/>           <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="kent_borowitz"/>           <!-- unwanted; remove                                           -->
      ...
      <xsl:apply-templates select="dc:description"           mode="kent_borowitz"/>           <!-- create dcterms:description                                 -->
      ...
      <xsl:apply-templates select="dc:source"                mode="kent_borowitz"/>           <!-- create dc:relation                                             -->
  ...
  <xsl:template match="dc:format" mode="kent_borowitz"/>

  <xsl:template match="dc:date" mode="kent_borowitz">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="substring-before(., 'T')"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="kent_borowitz">
    <xsl:if test="contains(., 'kent.edu/borowitz/')">
  ...
  <xsl:template match="dc:description" mode="kent_borowitz">
    <xsl:choose>
      <xsl:when test="contains(., 'thumbnail.jpg')">
  ...
  <xsl:template match="dc:source" mode="kent_borowitz">
