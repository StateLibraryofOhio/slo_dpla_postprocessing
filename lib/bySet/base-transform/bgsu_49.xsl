...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">BG News</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP FIELDS -->
      <xsl:apply-templates select="dc:identifier"            mode="bgsu_49"/>                 <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dc:format"                mode="bgsu_49"/>                 <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:subject"               mode="bgsu_49"/>                 <!-- create dcterms:subject                                     -->
      ...
      <xsl:apply-templates select="dcterms:isPartOf"         mode="bgsu_49"/>                 <!-- create dc:relation                                         -->
  ...
  <xsl:template match="dc:format" mode="bgsu_49">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != 'application/pdf' and normalize-space(.) != ''">
  ...
  <xsl:template match="dc:subject" mode="bgsu_49">
    <xsl:element name="dcterms:subject" namespace="http://purl.org/dc/terms/">
      <xsl:value-of select="normalize-space(replace(., '[ ]*--[ ]*', '--'))"/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:identifier" mode="bgsu_49">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:isPartOf" mode="bgsu_49">
    <xsl:for-each select="tokenize(., ';')">
