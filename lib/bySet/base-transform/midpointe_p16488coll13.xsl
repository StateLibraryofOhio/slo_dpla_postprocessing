...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Marion G. Warner Photograph Collection</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:date"                  mode="midpointe_p16488coll13"/>  <!-- create dc:date                                             -->
      <xsl:apply-templates select="dc:format"                mode="midpointe_p16488coll13"/>  <!-- create dc:format                                           -->
      <xsl:copy-of         select="dcterms:spatial"          copy-namespaces="no"/>           <!-- create dcterms:spatial                                     -->
      <xsl:apply-templates select="dc:coverage"              mode="midpointe_p16488coll13"/>  <!-- create dcterms:spatial                                     -->
      ...
      <xsl:apply-templates select="dc:source"                mode="midpointe_p16488coll13"/>  <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dc:coverage" mode="midpointe_p16488coll13">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dc:format" mode="midpointe_p16488coll13">
    <xsl:for-each select="tokenize(., ';')">
