...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">State Library of Ohio World War II Collection</xsl:element>              <!-- create dcterms:isPartOf -->

      <!-- REQUIRED ODN-MAP fields -->
      <xsl:apply-templates select="dc:identifier"            mode="ohmem_p16007coll21"/>      <!-- create edm:isShownAt, edm:preview, and dcterms:identifier  -->
      ...
      <xsl:apply-templates select="dcterms:alternative"      mode="ohmem_p16007coll21"/>      <!-- create dcterms:alternative                                 -->
      <xsl:apply-templates select="dc:contributor"           mode="ohmem_p16007coll21"/>      <!-- create dcterms:contributor                                 -->
      <xsl:apply-templates select="dc:description"           mode="odn"/>                     <!-- create dcterms:description                                 -->
      <xsl:apply-templates select="dcterms:extent"           mode="ohmem_p16007coll21"/>      <!-- create dcterms:extent                                      -->
                                                                                              <!-- dcterms:identifier is created above as part of the edm:isShownAt transform -->
      <xsl:apply-templates select="dc:publisher"             mode="odn"/>                     <!-- create dcterms:publisher                                   -->
      <xsl:apply-templates select="dc:relation"              mode="ohmem_p16007coll21"/>      <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:issued"           mode="ohmem_p16007coll21"/>      <!-- create dc:relation                                         -->
      <xsl:apply-templates select="dcterms:created"          mode="ohmem_p16007coll21"/>      <!-- create dc:relation                                         -->
                                                                                              <!-- dc:rights is created above as part of the edm:rights transform -->
      <xsl:copy-of         select="dcterms:rightsHolder"     copy-namespaces="no"/>           <!-- create dcterms:rightsHolder                                -->
      <xsl:apply-templates select="dcterms:temporal"         mode="ohmem_p16007coll21"/>      <!-- create dcterms:date                                        -->

      <xsl:apply-templates select="dcterms:abstract"         mode="ohmem_p16007coll21"/>      <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:isPartOf"         mode="ohmem_p16007coll21"/>      <!-- unwanted; remove                                           -->
      <xsl:apply-templates select="dcterms:requires"         mode="ohmem_p16007coll21"/>      <!-- unwanted; remove                                           -->
  ...
  <xsl:template match="dcterms:alternative" mode="ohmem_p16007coll21">
  ...
  <xsl:template match="dcterms:extent" mode="ohmem_p16007coll21">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
  ...
  <xsl:template match="dcterms:temporal" mode="ohmem_p16007coll21">
  ...
  <xsl:template match="dc:identifier" mode="ohmem_p16007coll21">
    <xsl:choose>
  ...
  <xsl:template match="dcterms:issued" mode="ohmem_p16007coll21">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dcterms:created" mode="ohmem_p16007coll21">
    <xsl:element name="dc:date" namespace="http://purl.org/dc/elements/1.1/">
      <xsl:value-of select="."/>
    </xsl:element>
  </xsl:template>

  <xsl:template match="dc:relation" mode="ohmem_p16007coll21"/>

  <xsl:template match="dcterms:abstract" mode="ohmem_p16007coll21"/>

  <xsl:template match="dcterms:isPartOf" mode="ohmem_p16007coll21"/>

  <xsl:template match="dcterms:requires" mode="ohmem_p16007coll21"/>

  <xsl:template match="dc:contributor" mode="ohmem_p16007coll21">
    <xsl:for-each select="tokenize(., ';')">
      <xsl:if test="normalize-space(.) != ''">
