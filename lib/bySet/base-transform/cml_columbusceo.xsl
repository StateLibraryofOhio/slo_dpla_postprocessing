...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="//oai_qdc:qualifieddc">
    <oai_qdc:qualifieddc
      ...
      <xsl:element name="dcterms:isPartOf" namespace="http://purl.org/dc/terms/">Columbus CEO</xsl:element>              <!-- create dcterms:isPartOf -->
      ...
      <xsl:apply-templates select="dc:rights"                mode="cml_columbusceo"/>         <!-- create edm:rights    and dc:rights                         -->
      ...
      <xsl:apply-templates select="dcterms:temporal"         mode="cml_columbusceo"/>         <!-- create dcterms:temporal                                    -->
      
      <xsl:apply-templates select="dc:source"                mode="odn"/>                     <!-- frequently unused; remove by default                       -->

      <xsl:apply-templates select="dcterms:accessRights"     mode="cml_columbusceo"/>
      <xsl:apply-templates select="dcterms:issued"           mode="cml_columbusceo"/>
      <xsl:apply-templates select="dc:coverage"              mode="cml_columbusceo"/>
      <xsl:apply-templates select="dcterms:medium"           mode="cml_columbusceo"/>
      <xsl:apply-templates select="dcterms:provenance"       mode="cml_columbusceo"/>
      <xsl:apply-templates select="dcterms:mediator"         mode="cml_columbusceo"/>

    </oai_qdc:qualifieddc>
  </xsl:template>

  <xsl:template match="dcterms:issued" mode="cml_columbusceo"/>
  <xsl:template match="dc:rights" mode="cml_columbusceo"/>
  <xsl:template match="dcterms:temporal" mode="cml_columbusceo"/>
  <xsl:template match="dcterms:medium" mode="cml_columbusceo"/>
  <xsl:template match="dcterms:provenance" mode="cml_columbusceo"/>
  <xsl:template match="dcterms:mediator" mode="cml_columbusceo"/>

  <xsl:template match="dc:coverage" mode="cml_columbusceo">
  ...
  <xsl:template match="dcterms:accessRights" mode="cml_columbusceo">
    <xsl:for-each select="tokenize(., ';')">
